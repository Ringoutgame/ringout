...
const SUITES = [
  ...
  { name: 'Football-Shell',   file: 'test_football_shell.js', expectPassed: 956 },   // Arena-Finalisierung + Review-Fixes: Rounded Rectangle B, gerades Tor, M1, Ballradius 25
  ...
  { name: 'Football-Tactical', file: 'test_football_tactical.js', expectPassed: 298 },   // Tactical 1v1 (2 Figuren, 1 Zug je Team/Runde) + sichtbare Modusauswahl der drei Produktmodi
  { name: 'Football-Elim',    file: 'test_football_elimination4.js', expectPassed: 1533 },  // Dev-Einstieg auf vier Startspieler: ZWEI LEBEN + adaptive Arena 4 -> 3 -> 2 -> 1 + fairer Respawn
  { name: 'Football-Elim5',   file: 'test_football_elimination5.js', expectPassed: 320 },
  { name: 'Football-ArenaKanon', file: 'test_football_arena_canonical.js', expectPassed: 138 },  // KANONISCHE ARENA B: eine Wandbeschreibung (FB_TWO_GOAL_SHAPE) fuer alle Zwei-Tor-Modi und dasselbe Schulterverhaeltnis radial (FB_RADIAL_SHAPE) fuer 3/4/5 Tore; prueft zusaetzlich, dass vom A/B/C-Labor kein Rest uebrig ist
  ...
  { name: 'Modus-und-Koerper', file: 'test_mode_bodies.js', expectPassed: 71 },  // STABILITY 01: Modus und Koerper wechseln nur gemeinsam. Der neutrale Football-Ball (Besitzer 5) hat ausserhalb von Arena Football keine Farbe - bliebe er stehen, bekaeme drawBall undefined, die Ausnahme verliesse die Bildschleife und es wuerde nie wieder gezeichnet. Geprueft werden alle 100 Uebergaenge des Vorspiels und die Ausschliesslichkeit des einen Wegs.
  { name: 'Football-EntscheidHUD', file: 'test_football_decision_hud.js', expectPassed: 145 },  // PASS 04: der Entscheidungszustand des Onlinespiels - Frist aus dem RAUM (v10 acht Sekunden, v9 sechs), bereit ist nur, wessen Terminal autoritativ im Raum steht, der Nenner sind die in dieser Runde Handlungsfaehigen, die Restzeit kommt aus der Serverzeit. Reine Anzeige: kein Schreibvorgang, kein Spielzustand, Ring Out unberuehrt.
  { name: 'Football-Team2v2', file: 'test_football_team2v2.js', expectPassed: 128 },  // TRUE TEAM 2V2: vier Identitaeten mit eigenen Kugeln, ein gemeinsames Fenster, gleichzeitiger Abschuss, Teamwertung nach Torseite (Eigentor eingeschlossen), First to 3
  ...
  { name: 'Football-FX',      file: 'test_football_fx.js',     expectPassed: 106 },    // Action-Feel 02: gedeckelte Effektliste, kurzlebige Formen, keine Spielzustandsberuehrung, RingOut unberuehrt  // Produktmodus Elimination: 5 -> 4 -> 3 -> 2 -> Sieger, dieselbe Architektur
  ...
  { name: 'Fixed-Timestep',  file: 'test_fixed_timestep.js', expectPassed: 203 },  // Gameplay laeuft in festen 60-Hz-Schritten, unabhaengig von der Bildwiederholrate
  ...
  { name: 'Join-Reliability', file: 'test_join_reliability.js', expectPassed: 50 },   // Shared: jeder Fernschritt des Beitritts hat eine Frist; danach entscheidet der Raum (eigener Sitz -> uebernehmen, sonst sichtbares Ende), nie ein haengendes 'Joining …', nie ein zweiter Sitz
  { name: 'Render-Recovery',  file: 'test_render_recovery.js', expectPassed: 58 },   // Shared: ein fehlgeschlagenes Bild beendet die Bildschleife nie; hoechstens 3 Wiederherstellungen je Fehlerklasse ueber den bestehenden Kontextverlust-Zyklus, dann kontrollierter Endzustand; Simulation laeuft weiter. Matchstart: Kontextverlust im Szenenaufbau abgefangen, Rueckgabe-Frist 10 s, Endzustand mit Neu-laden-Knopf + Ursachen-Code, lokaler Diagnosepuffer
  { name: 'R3D-Startup-Mem', file: 'test_r3d_startup_memory.js', expectPassed: 30 },   // Shared: Himmel + IBL explizit gebaut (PMREM-Zwischenpuffer, Wuerfel-Tiefenpuffer, HDRI-Quelle frei), flache Normal-Map ungenutzt, gestaffelter Erststart vor dem ersten Bild
  { name: 'RingOut-PackageB', file: 'test_ringout_package_b.js', expectPassed: 43 },   // Package B: RingOut FFA/Triple/Team Duel v12 - typisierter Skip fuer abwesende Sitze, passiv statt ausgeschieden, hoechstens 3 Versuche, v8 und Football unveraendert
  { name: 'Sanitize',         file: 'test_sanitize.js',       expectPassed: 24 },
  { name: 'Identity',         file: 'test_identity.js',       expectPassed: 45 },
  { name: 'ValidateRoom',     file: 'test_validateroom.js',   expectPassed: 49 },
  ...
  { name: 'Online-PhaseA',   file: 'test_online_phase_a.js', expectPassed: 422 },   // PROTOKOLL v8 PHASE A: ein Modusregister mit Sollbesetzung, Freigabetor fuer noch nicht fertige Onlinemodi, modusbewusste Lobby mit Soll/Ist - und der Nachweis, dass der Zugpfad unveraendert der v7-Pfad ist (kein v9-Mechanismus)   // reine Schema-/Vertragsschicht: Version, Raumtyp, Sitz/Koerper, kanonische Zugereignisse
  { name: 'Raum-Protokollweiche', file: 'test_online_room_protocol_routing.js', expectPassed: 84 },   // STUFE 2A: die Protokollfassung gehoert dem RAUM, nicht dem Client. Ein NEUER Raum bekommt sie aus seiner Konfiguration - Football mit der Lebensregel und drei bis fuenf Sitzen wird v9, alles andere bleibt v8, und der Waehler zieht dieselben Grenzen wie die Rules. Bestehende Raeume behalten ihre Fassung; derselbe ausgelieferte Client betritt beide Familien und ist in einem v8-Raum ausdruecklich KEIN v9-Client. Der Zugslot t bleibt v8 vorbehalten, die v9-Knoten bleiben v9-Raeumen vorbehalten - kein Mischbetrieb.
  { name: 'Online-V9',      file: 'test_online_v9.js', expectPassed: 349 },
  { name: 'Online-V10',     file: 'test_online_v10.js', expectPassed: 111 },
  { name: 'Online-V11',     file: 'test_online_v11.js', expectPassed: 172 },   // PROTOKOLL v11, NUR RULES: der bleibende Raum. Nicht der Raum haelt die Startbesetzung fest, sondern jede GENERATION ihre eigene, unveraenderliche Teilnehmerliste (g/<gen>/pt) - deshalb darf state zurueck nach 'lobby', duerfen Spieler zwischen zwei Matches gehen und kommen, duerfen die Sitze Luecken haben, und der Raum ist danach wieder oeffentlich auffindbar. Die Bereitschaft (g/<gen+1>/rd) gehoert dem KOMMENDEN Match; der Start ist ein atomarer Schritt aus gen, state und Teilnehmerliste. v8, v9 und v10 bleiben woertlich unberuehrt.   // PROTOKOLL v10, NUR RULES: dynamische Besetzung - config.cap ist die Hoechstbesetzung (5), seats die eingefrorene Startbesetzung (2-5); die Protokoll-Sitzmengen zaehlen bis seats; v9 bleibt woertlich; v10 ist oeffentlich auffindbar.   // V9.1/V9.2 GRUNDLAGE, NUR RULES: autoritative Turn-Eroeffnung (d/<turn>/{n,o}) und Commit-Terminal (c/<turn>/<seat>) - der freigegebene Client steht weiterhin auf Protokoll 8 und kann keinen v9-Raum anlegen. dazu der Reveal-Anker (ro/<turn>) und das write-once Reveal-Ergebnis (r/<turn>/<seat>) hinter der vollstaendigen Commit-Barriere. Die Rules koennen SHA-256 NICHT rechnen - die Bindung Hash<->Reveal und jede Spielwirkung kommen erst mit V9.3.
  { name: 'Online-V11-Client', file: 'test_online_v11_client.js', expectPassed: 248 },   // PLAYER LOOP V11: der Lebenszyklus des Clients - Teilnehmerliste je Generation (Luecken eingeschlossen), identitaetsgebundene Bereitschaft, atomarer Start aus gen+state+pt, Rueckweg in DIESELBE Lobby, Wiederkehr der oeffentlichen Liste. Kein Host, der startet; kein zweites Spielermodell neben der Elimination.
  ...
  { name: 'Football-Passiv-Marker', file: 'test_football_eviction_retry.js', expectPassed: 40 },   // HARDENING 01: der endgueltige Passiv-Marker nach der Rueckkehrfrist wird in v11 begrenzt nachgereicht - hoechstens drei Schreibversuche je Sitz und Generation, nie vor Ablauf der autoritativen Frist, abgebrochen mit Rueckkehr, Generationswechsel, Lobby und Austritt. v9/v10 und Ring Out unveraendert.
  ...
  { name: 'Rules',            file: 'test_rules.js',          expectPassed: 767 },   // v4: Seat-Ownership, Football-Raumtyp, typisierte Zuege (move/skip/remove) und die Eviction; Package B: RingOut-v12-Skip (36)
  { name: 'Fristen', file: 'test_timeouts.js', expectPassed: 33 },   // FRISTEN-INVENTAR: jede Zahl der Rules mit Pfad und Bedeutung, verglichen mit der Konstante des Clients, die dasselbe meint. Faellt aus, sobald Client und Server auseinanderlaufen - oder eine neue, hier nicht benannte Frist entsteht.
  ...
  { name: 'Football-Namen',   file: 'test_football_names.js', expectPassed: 92 },
  { name: 'Football-Tactical-Online', file: 'test_football_tactical_online.js', expectPassed: 100 },   // TACTICAL 1V1 ONLINE: v11-Raum mit zwei Sitzen, GLEICHZEITIGE Zuege (seit 2026-09-22) - Register/Raumfassung, Zugmodell je Variante (fbTacAbwechselnd nur 4-Ball), Koerperbesitz, Gatter fuer beide Sitze, Zugmenge (beide starten gemeinsam), zwei Clients, Schilder, Vertraege in HUD/Lobby/Rules
  { name: 'Football-Team2v2-Online', file: 'test_football_team2v2_online.js', expectPassed: 65 },   // TEAM 2V2 ONLINE: v11-Raum mit genau vier Sitzen (Blau 0/1, Rot 2/3), alle vier ziehen GLEICHZEITIG - Register/Raumfassung, Koerperbesitz (Sitz s: Koerper s), Eingabegatter (nur die eigene Figur, kein zweiter Zug), Zugmenge (vier Terminals, eine Aufloesung), Bereitschaft je Sitz aus dem Raum (Zaehler bereit/gesamt, Chipleiste), Namen und Teamfarben, Rules-Vertrag
  { name: 'Football-Tactical4-Online', file: 'test_football_tactical4_online.js', expectPassed: 104 },
  { name: 'Football-Hub-Vorschau', file: 'test_football_hub_preview.js', expectPassed: 102 },   // MODUSMENUE + RAUMVORSCHAU 2026-09-22: vier Karten (FFA, 1 VS 1, TEAM 2V2, TRAINING), 1 VS 1 oeffnet die Unterauswahl Tactical 1v1 / Tactical 4-Ball; Kartenbilder als deterministisches SVG aus den Spielkonstanten (gleiche Koerper wie placeBalls); der Onlinebildschirm eines Football-Raums zeigt die Football-Arena mit beiden Toren und den Koerpern des Modus (fbBuehne), Ring Out seine Ringplattform; Zurueck/Rejoin bauen sauber.   // TACTICAL 4-BALL 1V1 ONLINE: zwei Sitze, VIER Figuren je Sitz (Blau 0..3, Rot 4..7, Ball 8), abwechselnde Zuege wie Tactical - Register/Raumfassung, Praedikate (fbTactical fuer beide, fbTac4, fbTacFiguren), Koerperbesitz, gespiegelte Aufstellung, Eingabegatter, Zugmenge (dieselbe Figur darf wiederkommen), zwei Clients identisch ueber alle acht Figuren, Namen, Hub/Lobby/Rules-Vertraege
  { name: 'Football-Tactical-Spawn', file: 'test_football_tactical_spawn.js', expectPassed: 42 },   // TACTICAL 1V1 EROEFFNUNG: eigene Aufstellung 7.50/4.60 + 9.80/0.00 - Geometrie (Spiegelung, Abstaende, Bandenfreiheit, Torkorridor), die alte Aufstellung reproduziert den leichten direkten Eroeffnungstreffer, die neue nicht (Schusssuche auf der echten Physik, Blau == Rot), Konstanten-Vertraege
  { name: 'Football-Tactical4-Spawn', file: 'test_football_tactical4_spawn.js', expectPassed: 57 },
  { name: 'Football-EliteBot', file: 'test_football_elitebot.js', expectPassed: 139 },   // ELITE BOT (lokales 1v1): Verdrahtung, der Planer sieht nur oeffentlichen Zustand, verdeckte Eingaben aendern den Zug nicht, gleiche Saat -> gleicher Zug, Planung veraendert Stand/Koerper/Phase nicht, Suche ist begrenzt und in Scheiben bitgleich, Lebenslauf des Plans, Spielstaerke auf vier gestellten Lagen (echte Physik)   // TACTICAL 4-BALL EROEFFNUNG: eigene Konstante FOOTBALL_TACTICAL4_SPAWN (7.50/+-5.00, 9.40/0, 12.60/4.00) - Geometrie (neun Koerper, Spiegelung, Abstaende, Torkorridor, freie Bahnen), die alte gespiegelte Vier-Koerper-Aufstellung reproduziert den leichten Eroeffnungstreffer, die neue nicht (echte Physik, Blau == Rot), Team 2v2 und Tactical 1v1 unveraendert
];
