...
//
// Portiert aus dem RingOut-Quellrepository (0dd5858) in die gemeinsame App (RingOut-Delta
// B2). Angepasst an diese Codebasis: kein Online-Collapse (der Adapter ist offline/Bot),
// Replay-Stummschaltung in der hier vorhandenen Form, Kommentare mit artifacts/-Verweisen
// ausserhalb des Collapse-Laufzeitpfads sind erlaubt.
'use strict';
const fs = require('fs');
...
// ══ A) Quellen-Kausalitaet (statisch) ══
{
  ...
  t('A: kein Offline-Cue-Sheet im Collapse-Laufzeitpfad referenziert', !/collapse_sound_cues|artifacts\//.test(COLV_ALL + SFX_SRC));
  ...
  t('A: Replay/Rehydration stumm (fastForwardMatch schaltet soundOn ab und stellt es wieder her)',
    (() => { const a = HTML.indexOf('function fastForwardMatch('); const b = HTML.indexOf('\nfunction ', a + 10);
      const f = a < 0 ? '' : HTML.slice(a, b); return /const snd=soundOn; soundOn=false;/.test(f) && /soundOn=snd;/.test(f); })());
  ...
  t('A: Zwei-Stufen-Konstanten sind fest (kein Preview-Ternary; 60 s = 2 Stufen x 30 s)',
    /const MATCH_COLLAPSE_SECONDS=60, COLLAPSE_STAGE_COUNT=2, COLLAPSE_CYCLE_SECONDS=MATCH_COLLAPSE_SECONDS\/COLLAPSE_STAGE_COUNT, COLLAPSE_WARNING_SECONDS=10,/.test(HTML));
  ...
  t('A: Aufnahmen kommen aus dem Produktionsordner', /const COL_DIR='assets\/sfx\/ring_collapse\/';/.test(SFX_SRC));
  t('A: visueller Adapter ohne Online-Collapse-Abhaengigkeit (B2 offline/Bot)',
    !/onlineHasClock|onlineRemainMs|onlineCollapseCount/.test(COLV_ALL));
  t('A: Menue-Rueckweg und neues Match beenden die laufende Sequenz (collapseStop)',
    /function showMenu\(\)\{[\s\S]*?SFX\.collapseStop\(\);/.test(HTML) && /function resetCollapseTimer\(\)\{[\s\S]*?SFX\.collapseStop\(\);/.test(HTML));
  t('A: SFX.collapseStop ist nur an Menue, Matchstart und dem Hauptbruch selbst verdrahtet',
    (HTML.match(/SFX\.collapseStop\(\)/g) || []).length === 2 && (SFX_SRC.match(/collapseStop\(\);/g) || []).length === 1);
}
