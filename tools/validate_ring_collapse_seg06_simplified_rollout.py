# Ring-Collapse — SEG06 FIX ROLLOUT (correction pass 1): taper + seam
# harmonization for the corrected Segment06 root only. Segment03 stays the
# unchanged output of the original seg36 rollout (protected).
#
# Sources (read-only):
...
#         --python tools/validate_ring_collapse_seg06_simplified_rollout.py
import bpy
import math
import os
import time
...
SRC_FIX = os.path.join(ROOT, 'assets', 'ring_collapse', 'validation',
                       'seg36_gate', 'ring_collapse_seg06_simplified.glb')
...
GLB_OUT = os.path.join(VAL_DIR, 'ring_collapse_seg06_simplified_rollout.glb')
...
# replace the OLD seg06 with the corrected one (seg03 from the old pair
# stays as cracked-display context; it is NOT exported)
_old06 = bpy.data.objects.get('Segment06_Cracked_BALANCED')
_old06me = _old06.data
bpy.data.objects.remove(_old06, do_unlink=True)
bpy.data.meshes.remove(_old06me)
try:
    bpy.ops.import_scene.gltf(filepath=SRC_FIX, merge_vertices=False)
except TypeError:
    bpy.ops.import_scene.gltf(filepath=SRC_FIX)
assert bpy.data.objects.get('Segment06_Cracked_BALANCED') is not None
...
CRACKED[3] = bpy.data.objects.get(CRACKED_NAME % 3)      # old seg03 (context)
NEW = {n: bpy.data.objects.get(CRACKED_NAME % n) for n in NEW_CRACKED}
check('twelve_roots_imported',
      all(INTACT.values()) and all(CRACKED.values()) and all(NEW.values()) and
      CRACKED[3] is not None,
...
ALL_OBJS = ([(n, 'intact', INTACT[n]) for n in range(1, 7)] +
            [(n, 'cracked', CRACKED[n]) for n in OLD_CRACKED + [3]] +
...
for n, bdeg, nb in ((6, 308, 5), (6, 10, 1)):
    co = NEW[n]
...
PROXY_C = build_proxy([CRACKED[n] for n in OLD_CRACKED + [3]] +
...
NEED_LOOKUP = ([INTACT[n] for n in range(1, 7)] +
               [CRACKED[n] for n in OLD_CRACKED + [3]] +
...
NEIGH = {6: [(308, CRACKED[5]), (10, CRACKED[1])]}
for n in NEW_CRACKED:
    ob = NEW[n]
    iname = INTACT[n].name
    lk = LOOKUP[ob.name]
...
bpy.context.view_layer.objects.active = NEW[6]
...
check('glb_one_root', nodes == ['Segment06_Cracked_BALANCED'], str(nodes))
...
with open(os.path.join(ART_DIR, 'seg06_simplified_rollout_stats.json'), 'w') as fh:
