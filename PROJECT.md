...
**Zuletzt aktualisiert:** 2026-07-27 (Action-Clock Phase I+II gehärtet: atomarer Clock-Close auf dem Generations-Subtree, server-owned `eligibleSeats`, write-once Settlement-Reports — nicht produktionsbereit, kein Deploy; Online-v3 bleibt unverändert produktiv)
...
Ringout/
  ...
  firebase.rules.json# Server-seitige RTDB-Regeln (v3 publiziert; v4-Klauseln additiv, NICHT deployed)
  firebase.json      # Firebase-Projektkonfiguration (Rules, Functions-Quelle, Emulator-Ports)
  functions/         # Cloud Functions (europe-west1) — Action-Clock-Arbiter, NICHT deployed
    clock-core.js    #   Kernlogik: clockStart / clockClose / clockSettle (dependency-injiziert)
    index.js         #   onCall-Mantel: Auth-Pflicht, Fehler-Mapping
...
- **Offen:** Der Pair-3-Atlas (`validation/seg36_gate/textures/seg36_frac_*.png`) liegt weiterhin als PNG vor — **KTX2-/HQ-Finalisierung ist ein separater Folgetask**. Eine **Produktionsintegration ist nicht erfolgt**: `index.html`, Physik, Lockstep, Online und Firebase sind unberührt.
...
### Action-Clock: v4-Identity + Server-Arbiter — Phase I+II FERTIG, NICHT PRODUKTIONSBEREIT (2026-07-27)

**Fachlicher Vertrag (für alle Modi identisch):** 60 000 ms globale aktive Matchzeit, 7 000 ms maximale Entscheidungszeit je Aim-Phase; die globale Uhr läuft **nur** während der Entscheidungsphase und pausiert während Reveal/Physik/Result/Rundenwechsel/Menü; Timeout erzeugt einen verbindlichen No-Shot mit Impuls exakt 0 (Positionen und Geschwindigkeiten bleiben unangetastet); Cracked bei ≤ 10 000 ms Restzeit genau einmal, Expired bei 0 genau einmal; danach laufen Folgephasen **ungetimt** weiter — die Matchlogik bricht nie ab.

**Was heute läuft:** Der Vertrag ist **offline vollständig aktiv** (Bot-Training und lokaler Hotseat, Block `==COLLAPSE-CORE-START/END==` in `index.html`), inklusive der bestehenden Bot-Arena-Konsequenz (einmaliger Radius-Schrumpf `R*0.82`). **Online ist die lokale Uhr bewusst inert** (`clockActive() = clockEnabled && !online`) — die Online-Autorität liefert erst die Client-Anbindung in Phase III/IV.

**Warum serverseitig (Architekturentscheidung):** Ein erster Entwurf ließ die Online-Uhr clientautoritativ laufen (Knoten `clk`, von Clients per Transaction beschrieben). Das war unsicher — Restzeit, Deadline und die Crack-/Expired-Flags waren manipulierbar, und Rules können weder den korrekten Zeitabzug noch die Einmaligkeit inhaltlich prüfen. Diese Implementierung wurde **vollständig entfernt**; im Online-Pfad existiert kein clientgeschriebener Uhr-Zustand und kein spielentscheidender `performance.now()`/`serverTimeOffset` mehr.

**Identity (Phase I):** `index.html` bootet Firebase Anonymous Auth (`signInAnonymously`, `onAuthStateChanged` → `window.__FB_UID`, Event `fb-auth`) — **best-effort und nicht blockierend**. Anonymous Auth ist in der Firebase Console **noch nicht aktiviert**; bis dahin schlägt der Login still fehl und der produktive Online-v3-Pfad läuft unverändert.

**Identity-Boot ist vom Datenbank-Boot entkoppelt:** `firebase-auth.js` wird **dynamisch** importiert (`import(AUTH_MODULE_URL).then(…).catch(…)`). Ein statischer Import hätte bei blockiertem oder nicht ladbarem Modul die gesamte Modulauswertung abgebrochen — `window.FB` wäre nie gesetzt und das produktive v3-Online-Spiel damit tot. Fällt Auth aus, bleibt `window.__FB_UID` leer, v3 läuft unverändert und nur v4-Features sind inaktiv. Regressionsschutz: `tools/test_auth_boot.js` führt den echten Boot-Block aus `index.html` in vier Szenarien aus (Modul nicht ladbar / hängend / Provider deaktiviert / Auth aktiv).

**Protokoll v4 (Phase I, additiv):** `firebase.rules.json` akzeptiert `v` 3 **oder** 4; sämtliche v4-Klauseln sind als `v !== 4 || (…)` formuliert, alle v3-Pfade bleiben unverändert und ohne Auth nutzbar. In v4 gebunden an `auth.uid`: `players/<seat>` (neues Pflichtfeld `uid`), Presence `p/<seat>`, Turn-Slots (eigener Seat **und** `clock.phase==='aim'` **und** Turn-Bindung **und** Seat ∈ `clock.eligibleSeats` **und** `now <= clock.deadlineAt`), `state`/`seats` (Host) sowie `gen` (nur eingetragene Spieler). Der Clock-Anker liegt **generationsgebunden** unter `rooms/<code>/g/<gen>/clock` und ist für Clients gesperrt (`".write": false` **und** `".validate": false`); der frühere raumweite Pfad `rooms/<code>/clock` existiert nicht mehr und fällt unter `$other` (`".validate": false`).

**Server-Arbiter (Phase II):** neues Verzeichnis `functions/` mit `clock-core.js` (Kernlogik, dependency-injiziert über `{db, now}`) und `index.js` (dünner `onCall`-Mantel: Auth-Pflicht, Fehler-Mapping, Region `europe-west1`, Node 20). Drei Endpunkte:
- `clockStart` — eröffnet write-once je Generation die erste Aim-Phase (`remainingMs = 60000`, `eligibleSeats` aus dem v4-Roster).
- `clockClose` — schließt die Aim-Phase, wenn alle Slots der zugberechtigten Seats vorliegen **oder** die Server-Deadline erreicht ist; füllt fehlende Slots als verbindlichen No-Shot (`{ns:'stand'}`) und schreibt `remainingMs`, `cracked`, `expired`, `closedAt` und `settleDeadlineAt` fort — alles in **einer** Transaction (s. u.).
- `clockSettle` — sammelt je zugberechtigtem Seat einen **write-once** Report `{hash, next}`. **Nur ein vollständiges, widerspruchsfreies Quorum** aus `eligibleSeats` eröffnet genau einmal die nächste Phase; abweichende Hashes **oder** abweichende `nextEligibleSeats` beenden die Phase kontrolliert als `finished/desync`, eine leere Folgemenge als `finished/complete`. Nach `settleDeadlineAt` gilt: ist das Quorum unvollständig, endet die Phase deterministisch als `finished/settlement_timeout` — **ein Teilreport darf nie eine Folgephase eröffnen** und `turn`/`phaseId`/`eligibleSeats` bleiben unverändert.

**Atomarer Abschluss (Kern der Härtung, 2026-07-27):** Clock und Turn-Slots derselben Generation liegen im selben Subtree `rooms/<code>/g/<gen>`, und `clockClose()` läuft als **genau eine** RTDB-Transaction auf genau diesem Knoten: Sie validiert `phaseId`/Generation/Turn/Phase, prüft die Serverzeit gegen `deadlineAt`, liest die Slots **innerhalb** der Transaction, füllt alle fehlenden, rechnet `remainingMs` fort, setzt `cracked`/`expired`/`closedAt`/`settleDeadlineAt` und wechselt auf `resolving`. Vorher war das zweistufig (Slot-Transactions, danach Clock-Transaction) — ein Beobachter konnte eine `aim`-Phase mit bereits gebuchten No-Shots sehen, und ein Absturz dazwischen hätte einen halb gebuchten Zug hinterlassen. Zwei parallele `clockClose()`-Aufrufe konvergieren jetzt: einer schließt ab, der andere ist ein idempotenter No-op. Bewiesen wird das mit einem Listener, der jeden beobachtbaren Zustand protokolliert.

**Zugberechtigung statt Presence:** `clock.eligibleSeats` (normalisierter CSV-Schlüssel, z. B. `'0,2'`) ist **server-owned** und für die Dauer einer Phase unveränderlich. Die erste Phase leitet ihn aus dem v4-Roster ab, jede Folgephase **ausschließlich aus dem Settlement-Konsens** — jeder Report trägt `nextEligibleSeats`, das normalisiert, sortiert, eindeutig und Teilmenge der aktuellen Menge sein muss. Damit blockiert ein eliminierter, aber noch verbundener Seat die nächste Runde nicht, bekommt keinen Timeout-No-Shot und kostet die globale Uhr keine 7 Sekunden. **Presence entscheidet über Timer-No-Shots gar nichts mehr:** ein vor der Transaction gelesener Presence-Snapshot ist per Definition veraltbar, ein Reconnect zwischen Read und Write hätte einen fremden `{ns:'left'}`-Sentinel erzeugt. Fehlende Slots werden deshalb **immer** als `{ns:'stand'}` gebucht; die Leave-Eliminierung außerhalb des Timer-Timeouts bleibt bewusst offen (Phase III/IV, s. u.).

**Grenze des Verfahrens (wichtig):** Settlement-Konsens **erkennt** Desyncs, ist aber **kein kryptografischer Beweis einer korrekten Simulation**. Einigen sich alle zugberechtigten Clients auf dasselbe falsche Ergebnis, übernimmt der Server es. Nach `settleDeadlineAt` wird **nie** aus einem einzelnen oder teilweisen Report fortgefahren: ein unvollständiges Quorum endet deterministisch als `finished/settlement_timeout` (reihenfolgeunabhängig — ein manipulierter Einzelreport kann ehrliche Seats nicht aus der nächsten Phase ausschließen), ein vollständiges Quorum wird normal ausgewertet (Konsens → nächste Phase, Widerspruch → `finished/desync`). Die Entscheidung fällt vollständig innerhalb der Clock-Transaction; `finished/desync` und `finished/settlement_timeout` sind terminal und idempotent.

**Nicht offensichtlich, daher dokumentiert:** Alle Zeitpunkte (`startedAt`, `deadlineAt`, `crackAt`, `expiresAt`, `closedAt`, `settleDeadlineAt`) sind **absolute, vorberechnete Serverzeiten**. Dadurch braucht der Arbiter **keinen Scheduler und keine geplante Deadline-Funktion**: ein beliebiger Teilnehmer darf `close`/`settle` jederzeit anstoßen, der Server validiert lazy gegen seine eigene Uhr, und die RTDB-Transaction erzwingt genau einen Abschluss je Phase. Der Timeout-No-Shot ist bewusst **kein** Leave-Sentinel: er trägt eine eigene Kugel des Seats, der Spieler bleibt im Match. `loadRoom()` liest gezielt nur `v`, `state`, `config/fmt`, `seats`, `gen` und `players` — nie den ganzen Raum inklusive wachsender `g/<gen>`-Historie.

**Tests:** 17/17 Client-/Rules-Suiten grün (Rules 241, Ring-Collapse/Action-Clock 251, Auth-Boot 15). `tools/test_action_clock.js` — **119/119** Asserts, treibt den Produktions-Arbiter mit injizierter Zeit gegen einen echten RTDB-Emulator mit den produktiven Rules, inklusive REST-Prüfungen der Client-Sicht; im Testpfad gibt es **keine festen Wartezeiten** (deterministische Listener-/Promise-Synchronisation; einzig der Emulator-Start wird per Readiness-Polling abgewartet). Abgedeckt: atomarer Close bei fünf Seats (Listener-Beweis, Exception vor der Transaction, zwei parallele Close), Commit gegen Timeout (auch parallel), Reconnect/Presence (kein `left` aus veraltetem Snapshot, Reconnect während Close und während `resolving`, beliebige Settle-Reihenfolge), Eliminierung (FFA und Team-Duel), Settlement (write-once, idempotenter Retry, Desync bei Hash **oder** `next`, parallele Settle) sowie die komplette Post-Grace-Matrix (Einzelreport → `settlement_timeout`, parallele widersprüchliche Reports, Quorum über die Grace-Grenze hinweg, volles widersprüchliches Quorum → `desync`, Poke ohne Reports, Terminal-Idempotenz). Die Java-Auflösung ist portabel (`JAVA_HOME` → `java` aus `PATH` → dokumentierter lokaler Fallback → klarer Fehler; **kein stiller Skip**, JDK 21 wird verifiziert). Die Suite ist **bewusst nicht** im CI-Runner registriert (sie braucht JDK 21 + globales `firebase-tools`, die reine Node-CI liefe rot) — Aufruf über `npm run test:arbiter`. **Vor Merge/Produktion muss sie in CI integriert werden.**

**Offen vor Produktion:** Client-v4-Anbindung an den Arbiter (Phase III/IV) · Leave-/Disconnect-Eliminierung und Quorum-Reduktion außerhalb des Timer-Timeouts (nur transaction-sicher und serverseitig aus der B2-Grace-Semantik ableitbar — bis dahin bucht `clockClose()` immer `stand`) · **UID-Eindeutigkeit über mehrere Seats** (die Rules verhindern derzeit nicht, dass dieselbe `auth.uid` mehrere Seats belegt; `seatOfUid` löst auf den ersten Treffer auf — vor Phase-III/IV-Produktion zu lösen) · **Wachstum der Generation-Transaction** (`clockClose()` transaktioniert `g/<gen>` inklusive der Turn-Historie; die Payload wächst mit jedem Turn und muss vor Produktion auf Wachstum/Payload geprüft bzw. strukturell begrenzt werden) · Anonymous Auth in der Console aktivieren · Blaze-Plan und Functions-Setup · Deploy von Rules/Functions/Hosting (Reihenfolge: Functions → Rules → Client) · CI-Integration der Arbiter-Suite · Härtung des `gen`-Bumps (ein Rematch-Bump mitten im Match setzt die Uhr auf 60 s zurück) · App Check und Rate-Limiting als dokumentierte Folgethemen. **Kein Deploy erfolgt**, keine Asset-Änderung, **kein sichtbares Segment-Fallen** und keine Ring-Collapse-Integration.
