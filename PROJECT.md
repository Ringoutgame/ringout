...
**Zuletzt aktualisiert:** 2026-09-25 (**STARTSEITE VORLAGEN 1+2** — Arena-Blickfang gross (`.fbHeroImg` 700/390, weiche Masken), Himmel mit Sonnen-/Blau-Ebene per CSS, Panel-Goldecken (`.fbPanel::after`), Goldglas-CTA, Schnellstart-Kugeln aus `hero_football.webp`; zuvor **STARTSEITE FEEDBACK** — Kartenklick waehlt nur (`fbHubKarte`), der Spielen-Knopf oeffnet; Sprache ohne gespeicherte Wahl = Geraetesprache (`geraeteSprache`); Marke mit `translate="no"`; Himmel aus Tageswolken; zuvor **STARTSEITE NACH VORLAGE** — Gestaltung unter `body.roAus` (Himmel `fb_home_sky.webp`, Blickfang, Titel, Panels `.fbPanel`), neue Schnellzugriffe `fbHomeOnlineOeffnen`/`fbHomeRaeume`/`fbHomeSchnellstart` nur auf bestehenden Wegen; `forgetRoom` aktualisiert die Rueckkehrleisten; zuvor **RING OUT: DEV + REVANCHE GESCHLOSSEN** — `ringoutSpielbar()` ohne `DEV_MENU`, Dev-Start und Dev-Online-FFA gesperrt, `rematchErlaubt()` sperrt die Revanche nach Ring-Out-Partien; zuvor **RING OUT AUS DEM ANGEBOT** — `RINGOUT_SPIELBAR=false`: Startseite ohne Spielwahl, Arena Football ist der Einstieg; keine neuen Ring-Out-Raeume, eigene Sitze bleiben erreichbar; Code erhalten, `?dev=1` oeffnet Ring Out fuer die QA; s. Abschnitt „Ring Out vorerst nicht im Angebot"; zuvor **RINGOUT ONLINE FEATURE PARITY** — live seit 2026-09-25 (Rules deployt, Pages `a936b51e`, Zwei-Client-Produktionsnachweis 54/0): jeder neue RingOut-Raum ist **Fassung 13** (`RINGOUT_PARITY_FASSUNG`), 8 und 12 bleiben lesbarer Bestand. Der **Two-Stage-Collapse** ist online autoritativ: `t/<turn>/ca` und `t/<turn>/cc` sind die Serverstempel der Planungsphase (write-once, Rules: `=== now`), aus denen jeder Client dieselbe verbrauchte Planungszeit faltet; `t/<turn>/cs` ist die Stufe 1..2, die VOR der Physik dieses Zuges gilt. Sie darf nur ein verbundener Sitz schreiben, der in diesem Zug noch nicht abgegeben hat — deshalb kann der Zug nicht geschlossen sein, bevor die Marke steht, und jeder Snapshot mit allen Abgaben enthaelt sie zwingend (Knoten-Snapshots sind in sich konsistent). Angewandt wird sie ueber `onCollapseExpire()`, also ueber genau eine Collapse-Semantik. Die **Rescue Wall** laeuft online ueber `g/<gen>/rw/<turn>/<seat>` (write-once, uid-gebunden, `{k:tick,s:segment}` oder `{p:true}`); eine spaet eintreffende Wand wird vom Abschussabbild an nachgerechnet (`simSnapshotTake`/`simReplayTurnTo`, uebernommen aus `feat/protocol-v4-bc @ 0dd5858`), und der Zug endet erst, wenn jeder Sitz, der setzen konnte, seine Aussage abgegeben hat (`roWaZugEnde`/`roWaHalt` — bis dahin ruht die Physik). Der Wandbestand wird aus den bestaetigten Waenden ABGELEITET, nie mitgeschrieben. Module: `roCol*` (Collapse) und `roWa*` (Wand) beim Onlinezug; s. Abschnitte „Online-Collapse" und „Online-Rescue-Wall"; zuvor **PHANTOMKLANG + 2D-BLITZ** — `fbSfxImpact` schweigt unter `fbVorausAn()`, die Karte TRAINING sichert die Szene vor VS BOTS; s. Abschnitte „Planung klingt nicht" und „Szene vor dem Spiel"; zuvor **TORFUNKEN** — der dekorative Pfosteneffekt `fbFeelPost` ist entfernt, der Pfostenklang bleibt; s. Abschnitt „Pfosteneffekt entfernt"; zuvor **ELITE BOT 1V1** — ein lokaler Planer fuer Arena Football, der mit der echten Physik vorausrechnet und nur oeffentlichen Zustand sieht; s. Abschnitt „Arena-Football · ELITE BOT"; zuvor **HUD-SPRACHFLACKERN** — die Zugzeile wird nicht mehr je Bild neu aufgebaut und ist von der Browseruebersetzung ausgenommen; zuvor **TACTICAL 4-BALL: RAUTE** — die Eroeffnungsaufstellung ist eine Raute ohne jeden direkten Eroeffnungstreffer; s. Abschnitt „Tactical 4-Ball 1v1"; zuvor **TACTICAL 1V1 GLEICHZEITIG** — beide Sitze ziehen in jeder Runde zur selben Zeit, Tactical 4-Ball bleibt abwechselnd; s. Abschnitt „Tactical 1v1 online"; zuvor **NAMENSSCHILDER**: keine mehr ueber den Figuren in Tactical 1v1 und Tactical 4-Ball, die Namen stehen oben im Spielstand; s. Abschnitt „Namensschilder ueber den Figuren"; zuvor **ARENA FOOTBALL IST DAS HAUPTSPIEL** — im Spielumschalter zuerst und voreingestellt, Ring Out zweiter und vollstaendig; s. Abschnitt „D1v2 Game Hub"; zuvor **STARTSEITEN-HERO** — der Arena-Football-Hub zeigt die Football-Arena der gewaehlten Karte, Ring Out seine Ringplattform; s. Abschnitt „Modusmenue und Raumvorschau"; zuvor **FOOTBALL-OPTIK** — Raumvorschau ohne Nebel, hoeheres Arena-Fenster, Tactical-Karten aus der Team-2v2-Aufnahme; s. Abschnitt „Modusmenue und Raumvorschau"; zuvor **MODUSMENUE + RAUMVORSCHAU** — vier Arena-Football-Karten FFA / 1 VS 1 / TEAM 2V2 / TRAINING, 1 VS 1 mit Unterauswahl Tactical 1v1 / Tactical 4-Ball, Premium-Bildkarten (Renderer-Aufnahmen; neues Tactical-4-Ball-Bild als Komposit), Football-Arena im Arena-Fenster des Onlinebildschirms; s. Abschnitt „Modusmenue und Raumvorschau"; zuvor **ARENA FOOTBALL TEAM 2V2 ONLINE** — v11-Raum mit Modus `team2v2` und genau vier Sitzen auf derselben Rundenmaschine wie die Lebensregel: Blau = Sitze 0/1, Rot = 2/3 (`fbTeam2Side`, nirgends gespeichert), ein Sitz = genau eine Figur (`fbV9IdxGehoert`: idx = Sitz), alle vier ziehen GLEICHZEITIG, Auflösung bei vier Terminals oder Fristschluss, Teamstand nach Torseite, Erster bis 3. Das Hotseat-Fenster `fbShared` gilt online nicht mehr (jeder greift nur seinen Sitz); Bereitschaft „2/4" und Chipleiste lesen die autoritativen Terminals des Raums (`fbV9HudStand.sitzBereit` → `fbTeam2BereitOnline`). Rules: Modus team2v2 mit Sitzzahl 4, Wirt auf 0 oder 2, Start nur mit allen vier, Zugslots sitzgebunden. Zuvor: **ARENA FOOTBALL TACTICAL 1V1 ONLINE** — v11-Raum mit Modus `tactical` und genau zwei Sitzen auf derselben Rundenmaschine wie die Lebensregel; neu ist genau eine Regel: je Runde zieht EIN Sitz (`fbTacAktivSitz(turn,gen) = (turn+gen+1)%2`, Sitz 0 eroeffnet, jede Runde und jedes Rematch wechselt), der andere traegt ein automatisches `pass`. Koerperbesitz `fbV9IdxGehoert` (Lebensregel idx = Sitz, Tactical Sitz s -> 2s/2s+1) an Commit, Enthuellung, Paarpruefung und Geheimnisform; Rules erzwingen Zugformel und Besitz. `fbV9LebenNeueRunde` oeffnet ohne laufendes Match keine Runde mehr. Rules manuell deployt, Pages `de255ab6`, Produktions-Rauchtest 37/0. Zuvor: **ARENA FOOTBALL NAMENSSCHILDER** — jede Figur traegt den eingegebenen Namen ihres Spielers. Gezeichnet wird mit derselben Zeichnung wie in Ring Out (`drawBall` Abschnitt 6, Canvas-Text ueber `OP()`); neu ist allein `nameLabelOn(b)` als einzige Sichtbarkeitsstelle: Ring Out unveraendert (nicht in der Simulation), Football durchgehend inkl. Physik, ohne Schild bleiben der neutrale Ball, das Menue-Vorspiel und Tactical. Namensquelle bleibt `pName()` → `nameForSeat()` → `players/<seat>/name`, keine zweite Zuordnung. Zuvor: **JOIN RELIABILITY** — jeder Fernschritt des Beitritts laeuft ueber `joinFrist` (Lesen 8 s, Sitzanspruch 8 s je Schritt, Ruecknahme 4 s); eine abgelaufene Frist fuehrt nach `joinNachFrist`: eigener Sitz -> Uebernahme ueber `attemptRejoin`, sonst genau EIN automatischer zweiter Anlauf (`joinAutoLauf`) und danach ein sichtbarer, wiederholbarer Endzustand (`joinStuck`). `joinRoom` uebernimmt einen bereits eigenen, nicht aktiven Sitz, bevor es einen neuen beansprucht. Spuren im lokalen Diagnosepuffer (`joinDiag`). Zuvor: **MOBILE 3D STARTUP PASS 01** — `initR3D` baut Himmel und IBL explizit (`r3dBuildEnv`: eigener PMREM-Generator, der freigegeben wird; Himmelswuerfel ohne Tiefenpuffer; HDRI-Quelle nach der Umrechnung frei) und laedt die Arena-Texturen gestaffelt vor dem ersten Bild hoch (`r3dVorladen` + `compileAsync`). Die flache Marmor-Normal-Map des Arena-GLB wird ueber `R3D_FLAT_NORMAL_MAPS` uebersprungen. Messwerkzeuge und Belege: `artifacts/mobile_startup_opt/`. Zuvor: **SHARED MOBILE MATCH-START** — `initR3D` faengt einen Kontextverlust schon waehrend des Szenenaufbaus ab (frueher Listener mit `preventDefault`, Uebergabe an den bestehenden Zyklus), die Rueckgabe des Kontexts hat eine Frist (`R3D_RESTORE_WAIT_MS` = 10 s), danach Endzustand mit Neu-laden-Knopf und Ursachen-Code; waehrend eines Zyklus ist `#cv3d` verborgen. Lokaler Diagnosepuffer `r3dDiag` (40 Eintraege, `localStorage` `ringout_diag`, Anzeige `?diag=1`). Zuvor: **RINGOUT PACKAGE B** — RingOut FFA, Triple und Team Duel laufen in der Raumfassung **v12** (`RINGOUT_SKIP_FASSUNG`), Versus bleibt v8. Geht ein Teilnehmer, schreibt ein anderer, anwesender Sitzinhaber einen typisierten Skip `{k:'skip'}` in dessen Zugslot; die Rules erlauben das nur, wenn der Eigentuemer seine Abwesenheit selbst belegt hat (`p/<sitz>` fehlt oder `on:false` seit >= 30 s Serverzeit). Die Kugel bleibt passiv im Spiel. Der untypisierte Null-Sentinel der v8-Raeume bleibt verboten (Fund 2). Zuvor: **SHARED RENDER RECOVERY** — die Hauptschleife `loop()` treibt Simulation UND Darstellung; die Darstellung laeuft jetzt ueber die Grenze `presentFrame()`. Ein Render-Fehler beendet die Schleife nicht mehr, sondern laeuft in den bestehenden Kontextverlust-Zyklus (`r3dBeginRecovery` + `r3dRestore`), hoechstens 3 Wiederherstellungen je Fehlerklasse, danach der vorhandene Hinweis. `r3dRestore` setzt das Zeichenziel zuerst auf die Leinwand zurueck. Zuvor: **ONLINE-ZWEI-CLIENT-DESYNC 01 BEHOBEN** — der Beitretende fror im Online-Versus im ersten Zug in der Planungsphase ein, waehrend der Host normal weiterspielte. Ursache: `restorePresencePass()` las den **ganzen Raumknoten** `rooms/<code>`, und dieser Pfad deckt den laufenden Zugpfad `g/<gen>/t/<turn>` mit ab. Beim Gast faellt dieser Lesevorgang mit dem Armieren des Turn-Listeners zusammen — Firebase bediente den Listener aus dem Schnappschuss des laufenden Reads und meldete ihn **nie beim Server an**. Der Client hielt sich dabei fuer gesund: `turnUnsub` gesetzt, kein Abbruch, kein Overlay. Fix: **gezielte Feld-Lesevorgaenge** (`v`, `gen`, `players/<sitz>`, `p/<sitz>`, `g/<gen>/e/<sitz>`) statt des Raumknotens — keine Sleeps, keine Retries, keine Rules- oder Protokollaenderung. **2 × 10 Zwei-Client-Zuege synchron**, Runner **58/58**. Zuvor: **RINGOUT RESCUE WALL RW1 IST IMPLEMENTIERT UND LIVE-VALIDIERT** — reaktive Schutzwand im lokalen Bot-Training: waehrend die Kugeln rollen tippt der Spieler in den aeusseren Ring (62–145 % des AKTUELLEN Radius, schrumpft also mit jeder Collapse-Stufe mit) und setzt eines von zwoelf 30-Grad-Segmenten. **Drei Einsaetze je Seat und Match, hoechstens einer je Zug**, Lebensdauer **genau ein Zug**, Wirkung nur auf die Kugeln des Besitzers, Restitution 0,55. Die Kollision laeuft **vor** der Ring-Out-Auswertung, damit eine gehaltene Kugel nie als Grenzueberschreitung zaehlt. Alle Einhaengepunkte in `stepSim` sind `typeof`-abgesichert — Golden-Physik bitgleich, **Arena Football unveraendert**. Neue Suite `test_rescue_wall.js` mit **130/0**, Runner **58/58**. Laufzeit belegt ueber Stufe 0, 1 und 2 inklusive Kollisionsbeweis auf der kleinsten Arena. **Online ist NICHT Teil von RW1** (rw-Slot, v4-Turn-Autoritaet und Firebase Rules bleiben unberuehrt; RW2 nicht begonnen). Zuvor: **ARENA-FOOTBALL-MODUSHIERARCHIE H02** — Arena Football öffnet jetzt auf **vier Kategorien**: **1V1, 2V2, FFA und BOT TRAINING**. **ONLINE ist keine Hauptkategorie mehr**, sondern eine Spielumgebung am Ende des jeweiligen Moduspfads — der Spieler wählt erst, WAS er spielt, dann die Regel, wo nötig die Spielerzahl, und zuletzt LOCAL oder ONLINE. Ein einziger Navigationsschirm trägt alle Unterschritte; er kennt keine Spielregeln und startet nichts selbst, sondern löst die **vorhandenen** Startwege aus. Die Onlinefläche ist ehrlich gegatet: freigegeben ist allein **FFA Lives mit 3–5 Spielern**, alles andere ist sichtbar gesperrt statt vorgetäuscht. Bot-Training ist als Kategorie vorbereitet und steht auf *Coming Soon* — **kein Bot-Gameplay implementiert**. Navigationsprobe **30/0**, UI-Suiten **3687/0**, keine Seitenfehler. Produktcommit `133333a7ca15d6eac85abc34f651bb09e3eb3d8a`. Zuvor: **D1v2 GAME HUB VISUELL FINALISIERT** — die Startseite trägt jetzt zwei erstklassige Spiele unter der Dachmarke RINGOUT: **RING OUT** und **ARENA FOOTBALL**, jeweils mit eigener cinematischer Hero-Karte, eigener Modusleiste, eigenem Akzent und eigenem CTA. Alle **zehn** Moduskarten (fünf RingOut, fünf Arena Football) tragen hochwertige, produktgetreue Bildassets; die früheren flachen SVG-/Diagrammkarten sind **vollständig entfernt**. Die Football-Key-Art respektiert die Produktwahrheit: **offene Torrahmen, kein Netz, keine Rückwand, kein Torkorridor**. Mobiles und Desktop-Layout sowie die bestehende Navigation bleiben funktionsfähig. Belegt durch Browser-Probe **13/0** und die relevanten UI-Suiten **3687/0**. Produktcommit: `966c188f0f6ef2389920e40241d92f31d05f73d3`. Zuvor: **ONLINE V8 PHASE A ABGESCHLOSSEN** — bestätigt durch eine echte Mehrbrowser-QA gegen `ringout-87fbb`: vier eigenständige Browser, echtes Firebase-Projekt, **40/40** bestanden, keine Seitenfehler. Belegt am laufenden Produkt: Modus und Sollbesetzung im Raum, `hostUid` unabhängig vom Sitz, Teamwahl beim Erstellen wie beim Beitreten, Beschriftung B1/B2 blau gegen R1/R2 rot ohne Grün und Gelb — auch in der Datenbank —, Abweisung des fünften Clients, Rückkehr von Gast und Wirt auf ihre Sitze, gesperrter Start für Team 2v2 und unveränderte Einzelfarben der Lebensregel. **Der Zugpfad bleibt der v7-Pfad im Klartext**; Hash-Commit/Reveal und autoritative Frist sind v9 und **nicht begonnen**. Zuvor: **TEAM-2V2-EINSTIEG**: der Modus führt in den normalen Onlinebildschirm; die Seite wird beim **Erstellen** vor der Anlage und beim **Beitreten** erst nach Code, Prüfung und frisch gelesener Belegung gewählt. Bis zur Bestätigung wird nichts beansprucht; ein verlorenes Rennen bleibt in derselben Seite. Die Auswahlschirme liegen über dem Onlinebildschirm (`#fbOnTeamOv` z-index 140 gegen `.cover` 120). Zuvor: **HOST/SITZ-ENTKOPPLUNG (v8)**: `hostUid` an der Raumwurzel, bei der Anlage an `auth.uid` gebunden und danach ohne Schreibweg. `isHost()` ist die einzige Hostfrage; ohne Kennung gilt weiterhin Sitz 0 (v4–v7). Der Team-2v2-Ersteller sitzt auf dem ersten Sitz seiner gewählten Seite (Blau 0, Rot 2) und behält jede Lebenszyklus-Befugnis; der Zugpfad bleibt sitzgebunden. Zuvor: **TEAM 2V2 ONLINE-LOBBY**: zwei Teams als Ableitung aus dem Sitz (Blau 0/1, Rot 2/3), Teamwahl begrenzt die Kandidatensitze des atomaren Claims, Lobby mit festen Plätzen B1/B2 gegen R1/R2; ein nicht freigegebener Modus startet nicht. **Kein Rules- und kein Schema-Delta** — `config.cap` bleibt 4, das Team wird nirgends gespeichert. Zuvor: **ONLINE-PROTOKOLL v8 (Phase A)**: `config.mode` und `config.cap` stehen unveränderlich im Raum; zulässig sind nur classic/speed 2, team2v2 4, lives und timedffa 3–5. Der Start verlangt `seats === cap`, ein Sitz oberhalb der Sollbesetzung wird abgewiesen. Ein **Modusregister** (`FB_ONLINE_MODES`) ist die einzige Quelle für Kapazität, Beschriftung und **Freigabestand**; freigegeben ist online allein die Lebensregel. **Der Zugpfad `g/<gen>/t/<turn>/<seat>` behält exakt seine v7-Bedeutung** — Frist und Hash-Commit/Reveal kommen erst mit v9. Rules additiv: v4–v7 bleiben gültig. Zuvor: **die lokale Lebensregel teilt sich das gemeinsame Entscheidungsfenster**: alle aktiven Spieler entscheiden im selben 6-Sekunden-Fenster (`FOOTBALL_LIVES_SHOT_SECONDS`) und starten gleichzeitig, der Übergabeschirm entfällt; übernommen wurde nur das Fenster, **nicht** Phasenuhr oder DANGER, und die Regel (2 Gegentore) ist unverändert. `fbShared()` ist jetzt das Prädikat dreier Regeln; online bleibt die Lebensregel sequentiell, Protokoll v7 unberührt. Zuvor: **kanonische Wandform B**: eine Beschreibung (`FB_TWO_GOAL_SHAPE`) für Classic, Tactical, Team 2v2 und das Zwei-Spieler-Finale, dasselbe Schulterverhältnis radial (`FB_RADIAL_SHAPE`) für 3/4/5 Tore; das A/B/C-Labor ist restlos entfernt. Zuvor: **Geometrielabor A/B/C**: `?dev=1&arenaGeom=A|B|C` tauscht rein entwicklungsseitig die Wandform der Zwei-Tor-Arenen; ohne Parameter und online ändert sich nichts. Zuvor: **neuer lokaler Modus TRUE TEAM 2V2**: vier Identitäten B1/B2/R1/R2 mit je einer eigenen Figur, zwei Teams, First to 3 nach Torseite; ein gemeinsames 6-Sekunden-Fenster und gleichzeitiger Abschuss aus Timed FFA, Arena und Spawns unverändert aus Tactical. Zuvor: **Timed FFA ist simultan**: ein gemeinsames 6-Sekunden-Fenster fuer alle aktiven Spieler, ein gemeinsamer Start, kein Uebergabeschirm; Countdown gehoert dem Fenster (`FOOTBALL_FFA_WINDOW`), READY je Spieler im Chip. Zuvor: **Timed FFA zaehlt Bedenkzeit, nicht Ballzeit** — die Phasenuhr sinkt nur beim Zielen, dazu ein 6-Sekunden-Riegel je Zug mit Auto-Commit des anliegenden Vektors; grosser zentraler Countdown, Phasenuhr daneben. Dieselbe Uhr wie Classic Speed Match, dasselbe Gatter `fbDecisionWho()`. Zuvor: **Elimination hat zwei Regeln: LIVES und TIMED FFA** — Timed FFA ist rein lokal, zählt Gegentore in 60-s-Phasen und scheidet den Schlechtesten aus; Gleichstand führt in FFA DANGER SUDDEN DEATH. **Online bleibt ausschließlich Lives**, Protokoll v7 unverändert. Zuvor: **Arena-Football-Menümusik mit Energiekurve**: `FBTRACK` spielt `assets/audio/arena-football-menu.m4a` — leichter Einstieg bis 11,25 s, Aufbau bis 28,125 s, danach volle Energie; die Schleife umfasst nur die volle Phase (28,125 → 65,625 s), Menüpegel 0,30, Matchblende 0,70 s; das prozedurale Thema bleibt liegen, wird aber in jedem Bild auf `off` gehalten; Regeln, Physik, HUD und **Online-Protokoll v7** unverändert)
...
> Stand 2026-09-21: der Runner fasst **67 Suiten** zusammen, alle gruen (Rules 708/0). Die Zahlen der einzelnen Zeilen unten stammen aus aelteren Messungen und sind nicht nachgezogen; die beiden Online-Modussuiten sind aktuell.
...
| Football-Tactical-Online | `test_football_tactical_online.js` | 99/0 | grün |
| Football-Team2v2-Online | `test_football_team2v2_online.js` | 65/0 | grün |
| Football-Tactical4-Online | `test_football_tactical4_online.js` | 104/0 | grün |
| Football-Hub-Vorschau | `test_football_hub_preview.js` | 98/0 | grün |
...
| Rules | `test_rules.js` | 767/0 | grün |
...
**Regel für Lesevorgänge am Raum (seit Desync 01, 2026-09-17).** Ein Lesevorgang auf
`rooms/<code>` deckt den laufenden Zugpfad `g/<gen>/t/<turn>` mit ab. Wird ein Turn-Listener
registriert, während ein solcher Read unterwegs ist, bedient Firebase ihn aus dessen
Schnappschuss und meldet ihn **nie beim Server an** — der Sitz sieht von da an keinen
fremden Zug mehr, ohne dass der Client etwas davon merkt. `restorePresencePass` liest
deshalb nur noch die fünf Felder, die es prüft. Wer im Online-Pfad einen Raum liest, liest
gezielt; der ganze Knoten ist nur dort zulässig, wo sicher kein Match läuft.
...
**Football-Kontrakt.** Bis zu fuenf Teilnehmersitze `0..4`. Der neutrale Ball ist **kein**
...
**Startbesetzung: zwei bis fuenf.** Der Raum fasst hoechstens fuenf Spieler und zeigt das
auch (`n/5`); gestartet wird ab zwei. Die Zahl ist keine eigene Groesse, sondern das
bereits kanonische Startsignal `rooms/<code>/seats` — write-once, vom Host geschrieben,
und ab da die eingefrorene Teilnehmerliste dieser Generation. Auch ein Rueckkehrer liest
sie von dort und leitet sie **nicht** aus der aktuellen Praesenz ab; waehrend eines
Matches gehen Spieler verloren, die Startbesetzung aendert sich dadurch nicht. Im Client
gibt es dafuer genau eine Funktion (`fbElimPlayers()`, gespeist aus `fbElimStartN`);
Arenaform, Leben, Torslots, Aktivliste, Zugpflicht und HUD haengen daran wie bisher.

| Startbesetzung | Arena | Ablauf |
|---|---|---|
| 5 | Broad Rounded Pentagon | 5→4→3→2→Sieger |
| 4 | Rounded Square | 4→3→2→Sieger |
| 3 | Broad Rounded Triangle | 3→2→Sieger |
| 2 | Shouldered Wide | 2→Sieger |

Leere Lobbysitze sind **keine** Teilnehmer: keine Leben, kein Torslot, keine Kugel, keine
Zugpflicht, kein Eintrag in der Zughistorie und kein Platz in der Chipleiste. Ein Beitritt
in einen freien Sitz des laufenden Matches bleibt ausgeschlossen (Beitritt nur in der
Lobby, Rueckkehr nur auf einen Sitz `< seats`). Die Rules verlangen fuer das Startsignal
zwei bis fuenf, **alle** gezaehlten Sitze verbunden und oberhalb der Zahl keinen belegten
Sitz — sonst bliebe ein bereits Beigetretener ohne Startsignal zurueck.
...
erreichbar (ARENA FOOTBALL → ONLINE, 2–5 Spieler). **Tactical 1v1 ist seit 2026-09-21 online
spielbar** (Hub-Karte 1 VS 1 → Option TACTICAL 1V1 → Onlinelobby) — seit 2026-09-22 **gleichzeitig**
wie lokal (beide Sitze ziehen in jeder Runde, s. Abschnitt „Tactical 1v1 online"); nur **Tactical
4-Ball** zieht online abwechselnd. **Classic bleibt rein lokal.**

### Tactical 1v1 online — gleichzeitige Zuege auf der v11-Rundenmaschine (seit 2026-09-22)

**Zugmodell (seit 2026-09-22): GLEICHZEITIG.** Beide Sitze entscheiden in jeder Runde zur selben
Zeit; jeder waehlt frei EINE seiner zwei Figuren (`fbSel`, keine Rotation) und gibt genau EINEN
Zug ab; beide Zuege starten in derselben Simulation, alle fuenf Koerper kollidieren, die nicht
gewaehlten Figuren bleiben physisch. Kein aktiver Sitz, kein Eroeffner, kein Wechsel nach Tor
oder Rematch. Stehen beide Terminals, loest die Runde sofort auf; fehlt eines, schliesst die
Achtsekundenfrist es kanonisch (`late`/`skip`). Das Zugmodell haengt an genau EINEM Helfer:
`fbTacAbwechselnd()` (= online && Tactical 4-Ball) bzw. `fbTacGleichzeitig()` — gefragt von
`fbTacPassivRunde` (kein Auto-pass in 1v1), `canCommitInput`, `whoCanAim`, `fbTacticalRingLevel`,
`fbV9Wirken` (beide gueltigen Zuege starten) und `fbV9HudPaint`. `fbTactical()` bleibt die
Familienfrage (Besitz, Auswahl, Griffweite, Schilder). Nach der eigenen Abgabe ist die Eingabe zu
(`fbV9EigenAbgegeben()` in `whoCanAim` und am Auswahlring - bevor der Raum den Zug spiegelt; gilt fuer
alle gleichzeitigen Modi). HUD: dieselbe Zeile wie die Lebensregel plus
Zaehler „bereit/gesamt" (1/2, 2/2) aus `fbV9HudStand` — ohne Richtung/Kraft des Gegners. Rules: das
Zuggatter am Commit gilt nur noch fuer `tactical4`; in `tactical` schreibt jeder Sitz je Runde genau
einmal `move` (write-once, eigener Sitz, eroeffnete Runde, Frist), Koerperbesitz an der Enthuellung
unveraendert. Aufstellung unveraendert; gleichzeitige Eroeffnung gemessen (`test_football_tactical_spawn.js`
Abschnitt 5): kein Freitor — wer seine hintere Figur bewegt, oeffnet dem Gegner die Mitte, fuer beide
Seiten gleich. Pruefstaende: `tools/test_football_tactical_online.js` (99), `tools/test_rules.js`
Abschnitt 18; Laufzeit `artifacts/tactical02/lauf.js` (Matrix A–L, `--emu=1`), Live `artifacts/tactical02/live.js`.

**Bis 2026-09-22 galt das abwechselnde Modell, das heute nur noch Tactical 4-Ball traegt** (die
folgenden Punkte beschreiben Raum, Rundenmaschine und Besitz — sie gelten weiter; Zugformel,
passiver Sitz, Gatter und Zugansage gelten nur noch fuer 4-Ball):

- **Raum:** v11, `config.mode='tactical'`, `config.cap=2` (Register `FB_ONLINE_MODES.tactical`,
  `FB_TAC_SITZE`). Sitz 0 = Blau = Wirt (B1/B2 = Koerper 0/1), Sitz 1 = Rot (R1/R2 = Koerper 2/3),
  Koerper 4 = neutraler Ball — die Aufstellung von `placeBalls()` fuer Tactical. `fbRaumFassung`
  gibt 11 nur fuer genau zwei Sitze; `fbVarianteFuerModus` setzt die Variante an allen drei
  Einstiegen (Anlage, Beitritt, Rueckkehr). START nur zu zweit (`fbV11Starten`, `fbV11Lobby`).
- **Rundenmaschine:** unveraendert die der Lebensregel (d, c, ro, r, z, q, Achtsekundenfrist,
  Praesenz, `late`/`skip`/`remove`, Rueckkehr ueber die Historie). `fbV9LebenAn` laeuft fuer
  Elimination UND Tactical.
- **Die eine neue Regel — wer am Zug ist:** `fbTacAktivSitz(turn, gen) = (turn + gen + 1) % 2`.
  Kein Feld im Raum; Client und Rules rechnen dieselbe Formel (Rules: Rundenparitaet ueber die
  Endziffer `$turn.matches(/[02468]$/)`). Sitz 0 eroeffnet Generation 1; jede Runde wechselt die
  Seite, jedes Rematch (naechste Generation) wechselt, wer eroeffnet. **Nach einem Tor gilt dieselbe
  Regel:** es zieht, wer nicht geschossen hat — im Regelfall die Seite, die das Tor hinnehmen musste.
- **Der passive Sitz** traegt sein `pass` automatisch: `fbV9LebenBereit` gibt es der Runde von
  Anfang an mit; nach einem Neuladen in eine offene Runde reicht `fbV9EigenBekannt` es nach, sobald
  der eigene Slot bekannt ist (Haken in Schritt 2a der Ablaufsteuerung). Kein zweiter Spielerzug.
- **Gatter:** Client — `whoCanAim`/`canCommitInput` nur fuer den Sitz am Zug, Figurenring nur dort,
  `fbV9Wirken` startet nur den Koerper des aktiven Sitzes (zweiter Riegel). Rules — `c/<turn>/<seat>`
  nimmt `move` nur vom Sitz am Zug, `r/<turn>/<seat>/idx` nur einen eigenen Koerper.
- **Koerperbesitz an einer Stelle:** `fbV9IdxGehoert(seat, idx)` — Lebensregel `idx === seat`,
  Tactical `idx === 2s || 2s+1` — benutzt in `fbV9MakeCommit`, `fbV9NetWriteReveal`,
  `fbV9VerifyPair`, `fbV9SecretShapeOk`, `fbV9AcceptedOk` (jeweils typeof-geschuetzt fuer die
  Codec-Pruefstaende).
- **HUD:** dieselbe eine Zeile `#fbState` — `DEIN ZUG · <Rest>` fuer den Sitz am Zug, `GEGNER AM
  ZUG` (Klasse `wait`) fuer den anderen. Namen auf beiden Figuren eines Spielers (`nameLabelOn`).
- **Wertung:** unveraendert Classic/Tactical — Erster bis 3 (`FOOTBALL_WIN_SCORE`), Rematch im Raum.
- **Disconnect:** die v11-Passivitaet — ein getrennter Sitz wird in seinen Runden mit `skip`
  geschlossen, seine Figuren bleiben, er kann innerhalb der Rueckkehrfrist zurueck; der Rest spielt
  weiter und kann das Match beenden. Rueckkehr stellt Sitz, beide Figuren und die Runde aus der
  Historie wieder her (belegt: Neuladen mitten im Match).
- **Pruefstaende:** `tools/test_football_tactical_online.js` (72), `tools/test_rules.js` Abschnitt 18
  (+46); Laufzeit `artifacts/tactical01/lauf.js` (`--emu=1` faehrt den lokalen RTDB-Emulator mit den
  Rules aus der Datei, Produktions-Firebase fail-closed; ohne `--emu` das echte Backend). `mode='football'` wird an genau **fuenf** Stellen gesetzt: im lokalen
...
- **Modus:** Raum `config.mode='tactical4'`, `config.cap=2`, v11 (Register `FB_ONLINE_MODES.tactical4`,
  ...
  1 VS 1 → Option TACTICAL 4-BALL 1V1 (`fbDuelTac4Btn`, seit 2026-09-22; zuvor eigene Karte
  `cardFb4b`) fuehrt wie Tactical in die Lobby (`fbTactical4OnlineOeffnen`); Dev-Direktlink
  `?dev=1&fb=tactical4`. Die bestehenden
...
- **Eine Familie, kein zweites Werk:** `fbTactical()` gilt fuer beide Varianten, `fbTac4()` nur fuer
  ...
  Aufstellung. **Seit 2026-09-22 ist das abwechselnde Zugmodell das dieser Variante allein**
  (`fbTacAbwechselnd()`; Tactical 1v1 zieht gleichzeitig): Zugformel `fbTacAktivSitz(turn,gen)=(turn+gen+1)%2` (Sitz 0 eroeffnet Generation 1,
...
- **Aufstellung — die RAUTE (seit 2026-09-22, eigene Konstante `FOOTBALL_TACTICAL4_SPAWN`):**
  vier [x,y]-Paare in BR fuer Blau bei -x, Rot exakt gespiegelt, in der Reihenfolge der Form:
  **A1 Spitze vorn (4.25, 0)** auf der Torachse und am naechsten am Ball, **A2/A3 Seiten
  (7.00, -+8.00)** spiegelgleich auf halber Tiefe, **A4 hinten (9.75, 0)** auf der Torachse,
  8.25 BR vor der Torlinie. Eine echte Raute: Diagonalen 5.50 BR (tief) und 16.00 BR (breit),
  Schenkel gleich lang (8.46 BR), Seiten exakt auf der Mitte zwischen Spitze und Hinterfigur.
  ...
  **Warum so breit:** Front, Ball und gegnerische Front liegen auf einer Geraden, ebenso jede
  Seitenfigur, der Ball und die GEGENUEBERLIEGENDE Seitenfigur des Gegners. Ein leicht
  verzogener Schuss auf den Ball prallt dort ab; bei schmaler Raute (Seiten bei -+4.50) zeigt
  dieser Abpraller ins Tor (2-Grad-Fenster, kraftrobust bei 100 % und 85 %). Erst ab 8.00 BR
  Breite laeuft er am Tor vorbei. Der Raute fehlt der tiefe Aussenverteidiger der alten
  Aufstellung, der genau diesen Weg blockte - die Breite ersetzt ihn.
  **Gemessen** (`artifacts/tactical4-spawn-01/eroeffnung4.js`, echte Physik, 1-Grad-Raster,
  Kraft 100/85/70/55/40 %, alle acht Figuren): direkte Eroeffnungstreffer **0/0/0/0/0**
  (gestaffelte Vorgaengerin 2/1/0/0/0, erste Aufstellung 16 bei 100 % mit 17 kraftrobusten
  Winkeln), breitestes Fenster **0 Grad**, kraftrobuste Winkel **0**, Eigentore **0**, Blau und
  Rot Zahl fuer Zahl gleich. Es bleibt ein einzelner Mehrfachkontakt-Bankweg je Seitenfigur bei
  ausschliesslich 100 % Kraft (1 Winkel von 360).
  **Brauchbarkeit aller vier:** Ballkontakt im ersten Zug 14/11/11/7 % der Winkel, mittlere
  Ballverlagerung Richtung Gegnertor 8.4/4.4/4.4/8.5 BR. Die Hinterfigur steht bauartbedingt
  hinter der eigenen Spitze - sie ist die Aufbaufigur und spielt ueber sie; ihre Bahn dorthin
  ist frei. **Abstaende:** eigene Figuren >= 5.50 BR (Griffweite 2.8 BR), Spalt Spitze-Ball
  2.47 BR, Bandenabstand 4.70 BR, Torkorridor frei. Suite `tools/test_football_tactical4_spawn.js`
  haelt die FORM fest (Achse, Spiegelung, halbe Tiefe, gleiche Schenkel, Diagonalen) und faehrt
  die schmale Raute als Gegenprobe mit. Team 2v2 behaelt `FOOTBALL_TACTICAL_SPAWN`, Tactical 1v1
  `FOOTBALL_TACTICAL_1V1_SPAWN`. Arena und Kamerarahmung wie Tactical.
...
### Team 2v2 online — vier Menschen, zwei Teams, alle gleichzeitig (2026-09-21)

- **Raum:** v11, `config.mode='team2v2'`, `config.cap=4` (Register `FB_ONLINE_MODES.team2v2`,
  `FB_TEAM2_SITZE`). `fbRaumFassung` gibt 11 nur fuer genau vier Sitze; `fbVarianteFuerModus` setzt
  die Team-2v2-Variante an Anlage, Beitritt und Rueckkehr. START nur zu viert mit genau den Sitzen
  0,1,2,3 (`fbV11Starten`, `fbV11Lobby` mit fester Sollbesetzung `genau`). Hub-Karte TEAM 2V2 fuehrt
  direkt in die Lobby (`fbTeam2OnlineOeffnen`); die Seite waehlt jeder beim Anlegen/Beitreten
  (bestehende Teamfrage `fbTeamAsk`).
- **Sitz → Team → Figur (autoritativ, nirgends gespeichert):** Blau = Sitze 0/1 (B1/B2), Rot =
  Sitze 2/3 (R1/R2) — `fbTeam2Side(o)`, `FB_TEAM2_SEATS=[[0,1],[2,3]]`. Der Wirt sitzt auf dem ersten
  Sitz seiner gewaehlten Seite (Blau 0, Rot 2). Jeder Sitz fuehrt genau EINE Figur, Koerper
  `idx === seat` (`fbV9IdxGehoert`), Koerper 5 ist der neutrale Ball ohne Sitz. Namensschilder auf
  den vier Figuren in Teamfarbe (`nameLabelOn`, `ncol(colorSlot(owner))`), keines auf dem Ball.
- **Rundenmaschine:** unveraendert die der Lebensregel — d, c, ro, r, z, q, Achtsekundenfrist
  (`fbV9FristMs`, Fix 3fd2fd9 bleibt), Praesenz, `late`/`skip`/`remove`, Rueckkehr ueber die Historie.
  Alle vier ziehen gleichzeitig; aufgeloest wird, sobald vier Terminals stehen, sonst am Fristschluss.
  **Keine abwechselnden Zuege** — `fbTacAktivSitz` gilt nur fuer Tactical. `fbV9LebenAn` laeuft fuer
  Elimination, Tactical und Team 2v2.
- **Das Hotseat-Fenster ist online aus:** `fbShared() = fbTimed() || fbLives() || (fbTeam2() && !online)`.
  Lokal greift jede offene Figur (vier Menschen, ein Geraet); online greift jeder Client nur seinen
  Sitz (`whoCanAim`, `pickOwnBall(myPlayer)`), ein zweiter Zug in derselben Runde ist zu. Vorher war
  das Fenster online offen: ein Client konnte die Figur des Teamkollegen greifen, der Commit fuer den
  fremden Koerper scheiterte an `fbV9IdxGehoert`, und die eigene Runde blieb haengen.
- **Wertung:** unveraendert der lokale Vertrag — Teamstand nach Torseite (`footballGoalSide`, Eigentore
  ohne Sonderfall), Erster bis 3 (`FOOTBALL_WIN_SCORE`), Rematch im Raum (naechste Generation). Kein
  Einzelspielerstand, keine Leben, keine Phasenuhr.
- **HUD:** dieselbe eine Zeile `#fbState` mit angehaengtem Zaehler `<Rest> · bereit/gesamt` (Nenner =
  noch handlungsfaehige Sitze, aus `fbV9HudStand`); je Sitz `sitzBereit` (Terminal autoritativ im
  Raum). `fbV9HudPaint` hinterlegt die Sitzbereitschaft in `fbTeam2BereitOnline` und zeichnet die
  Chipleiste nur neu, wenn sie kippt; `renderTeam2Bar` liest sie (READY je Chip, eigener Chip
  `mine`, Namen statt B1/B2, Teamfarbe). Der lokale Phasentext (`fbOffen`) bleibt online leer.
- **Rules:** v11 erlaubt Modus `team2v2` mit Sitzzahl 4; Anlage nur mit `hostUid === auth.uid` und
  Wirt auf Sitz 0 oder 2; `pt` verlangt fuer team2v2 alle vier Sitze; `c/<turn>/<seat>` nur vom eigenen
  Sitz (Teamkollege, Gegner, fremder Sitz, Doppelzug, Sitz 4 abgewiesen), `r/<turn>/<seat>/idx === seat`,
  `skip` nur fuer belegt abwesende Sitze. Sicherheit liegt in den Rules, nicht in der UI.
- **Disconnect/Rueckkehr:** v11-Passivitaet — der getrennte Sitz wird mit `skip` geschlossen, seine
  Figur bleibt mit Namen im Feld, kein falscher Sieger; Rueckkehr stellt Sitz, Team, Figur, Name,
  Generation, Runde und Stand aus der Historie wieder her (belegt: Neuladen mitten im Match, keine
...
### Modusmenue und Raumvorschau (2026-09-22)

- **Vier Karten.** `FB_HUB_MODES` = `ffa` (`cardFbFfa`), `duel` (`cardFb1v1`, Titel 1 VS 1 /
  DE 1 GEGEN 1 / TR BİRE BİR), `team2v2` (`cardFb2v2`), `training` (`cardFbBot`, soon). Jede Karte
  nennt ihr Modusbild (`bild`). 1 VS 1 fuehrt ueber `fbHubOeffnen('duel')` in die Unterauswahl
  `#fbDuelOv` (dieselbe Bauweise wie der Bot-Dialog) mit `FB_HUB_DUEL` = `tactical`
  (`fbDuelTacBtn`) und `tactical4` (`fbDuelTac4Btn`); Titel/Unterzeile sind die Registerschluessel
  des Onlinemodus (`onModeTactical(S)`, `onModeTactical4(S)`). Eine Option schliesst die Ebene und
  ruft `fbHubOeffnen(key)` - denselben Weg wie eine Karte; Zurueck schliesst nur die Ebene und
  setzt keinen Zustand. Interne Schluessel, Rules, Raumfassung, Zugmodell: unveraendert.
- **Kartenbilder.** Die Arena-Karten tragen die freigegebenen Renderer-Aufnahmen (700x438 WebP):
  FFA `football_elimination`, 1 VS 1 `football_tactical`, TEAM 2V2 `football_team2v2`, TRAINING
  `football_classic`; die Optionen der Unterauswahl `football_tactical` (2 Blau + 2 Rot + Ball) und
  `football_tactical4` (4 Blau + 4 Rot + Ball) ueber dieselbe Buehnenklasse `.mshot` in `.vopt-art`.
  Beide Tactical-Bilder sind deterministische Komposite aus `football_team2v2.webp` (kanonische
  Schulter-Arena, goldenes Licht wie FFA; `artifacts/fb-menu-preview-01/tactical_karten.js`:
  Farberkennung der fuenf Koerper, Homographie Spielflaeche -> Bild aus der Vier-Koerper-Aufstellung
  vom 9.9., Marmor-Retusche und Kugel-Sprites aus demselben Bild, Lagen aus
  `FOOTBALL_TACTICAL_1V1_SPAWN` bzw. `FOOTBALL_TACTICAL4_SPAWN`) - dieselbe Bildfamilie, kein
  Stilbruch. Die urspruengliche Tactical-Aufnahme vom 9.9. (runde Arenaform, blauer Himmel) ist
  ersetzt. Ein am 22.9. versuchter SVG-Generator aus den Spielkonstanten wurde zurueckgenommen
  (wirkte schematisch); eine heutige Rohaufnahme des Renderers trifft die filmische Familie vom
  9.9. nicht (`kartenbilder_tactical.js`, auch mit dem Himmel des 9.9.-Codes).
- **Raumvorschau.** Das Arena-Fenster des Onlinebildschirms (`.ohero`) ist die echte 3D-Szene.
  `fbOnlineVorschau()` = Menue offen, `mode==='football'`, `fmt===FB_ONLINE_FMT`, `#online.show`;
  `fbBuehne()` = Match oder Vorschau. Daran haengen `footballView` (Deck, Banden, Tore, Ringplattform
  weicht), `fbFrame` (Football-Masse) und `fbFrameKey` (Neurahmung). Im Menue rahmt der Renderer
  auf das Fenster: Hauptpunkt in die Fenstermitte, Abstand aus Breite und Fensterhoehe
  (`fbHeroRect`, `FB_VORSCHAU_ZOOM=0.90`); das Arena-Fenster ist fuer Arena Football hoeher
  (`body.fbctx .ohero` 46 vh), und das Nebelband der Szene beginnt in der Vorschau erst hinter
  der Arena (`fbVorschauNebel`: near = max(2600, 1.35 x Distanz), Bandbreite unveraendert; Match
  und Startseite stellen 2600/8000 zurueck) - sonst stand die Fuenf-Tore-Arena der FFA bei ~5400
  Einheiten milchig im Dunst. Das Menue-Framing von Ring Out bleibt Zeichen fuer Zeichen. `fbOnlineEnter` stellt die Koerper (`setzeVorspiel(mode,fmt)`); Beitritt und
  Wiedereintritt taten das bereits. `updateMenuPreview` setzt die Football-Vorschau der Startseite
  auf die Variante der gewaehlten Karte (s. Startseiten-Hero). Ring Out: `fbBuehne()` false, Ringplattform wie bisher.
- **Startseiten-Hero (seit 2026-09-22).** `fbBuehne()` = `mode==='football'`: die Football-Arena steht
  auch auf der Startseite, sobald die Arena-Football-Karte gewaehlt ist. `updateMenuPreview` nimmt die
  Variante der gewaehlten Moduskarte (`fbHubVariante`: FFA -> `FOOTBALL_VARIANT_ELIM` = Fuenf-Tore-
  Startarena mit 5 + Ball, 1 VS 1 -> `FOOTBALL_VARIANT_TACTICAL` 2 + 2 + Ball, TEAM 2V2 ->
  `FOOTBALL_VARIANT_TEAM2`, TRAINING -> classic); `selectFbMode` ruft `updateMenuPreview` beim
  Kartenwechsel. Der Renderer hat dafuer einen eigenen Zweig (`menuVisible&&fbFrame`): die Arena sitzt
  im Band zwischen Titel und Hub-Tafel (Mitte bei 37 % Bildhoehe wie die Ringplattform) und darf es
  nach unten um `FB_HERO_BAND` 1.3 ueberragen (nahe Bande unter der Tafel wie der Rand der
  Ringplattform, Tore und Mitte frei; Telefon: die Breite bindet, Desktop: die Bandhoehe), Zoom
  `FB_VORSCHAU_ZOOM`, Nebelband hinter der Arena. Der Arena-Fenster-Zweig (`fbHeroRect`) greift nur bei
...
- **Ausgeliefert (2026-09-22):** HUD-Sprachflackern behoben, Pages `e6d1761e` (Commit `b4b584f`), Produktions-Rauchtest 14/0; davor Tactical-4-Ball-Raute Pages `99b36537` (Commit `6a591ee`), Produktions-Rauchtest 14/0, Emulator 62/0; davor Tactical 1v1 gleichzeitig — Rules-Deploy (live == HEAD) und Pages `bef307f0` (Commit `1e2f86f`), Live-Smokes T1v1 21/0 / T4 14/0; davor Arena Football als Hauptspiel Pages `8a361a51` (Commit `5c4eaae`), Live-Smoke 19/0, Emulator-Rejoin 26/0 (inkl. Ring-Out-Wiedereintritt); davor Startseiten-Hero Pages `bffcb7c8` (Commit `a269cf9`), Live-Smoke 18/0, Emulator-Rejoin 19/0; davor Optik-Fix (Vorschau ohne Nebel, Tactical-Karten in der Familie) Pages `28b38a73` (Commit `62b3d8c`), Live-Smoke 17/0; davor Nachbesserung Bildkarten Pages `b1a5f906` (Commit `dcfae62`), Live-Smoke 17/0; zuvor Pages `cb634e99` (Commit `be90493`), Live-Smoke 18/0,
...
- **Pruefstaende:** `tools/test_football_hub_preview.js` (98); Headless-Oberflaeche
  ...
  `artifacts/tactical4_01/live.js` (ueber Karte 1 VS 1 + Option).
...
### Arena-Football · TRUE TEAM 2V2 (Produktmodus, 4 Spieler — lokal am Hotseat; online seit 2026-09-21, s. „Team 2v2 online")
...
**Kein zweites Werk.** `fbShared() = fbTimed() || fbLives() || (fbTeam2() && !online)` ist das **eine** (Hotseat-)
...
RINGOUT bleibt die Marke; darunter stehen **ARENA FOOTBALL** und **RING OUT**. Arena
Football ist damit nicht länger die sechste RingOut-Moduskarte.

**Produktprioritaet (seit 2026-09-22): Arena Football ist das Hauptspiel.** Im Umschalter
steht `#cardFootball` zuerst/links und traegt beim Laden die Auswahlmarkierung `on`,
`#cardRingout` folgt zweiter/rechts; Gestaltung, Markierung, Umschaltverhalten und das
Zweispaltenraster sind unveraendert. Ein frischer Start waehlt Arena Football
(`let menuSel='football'`, angewendet vom Boot ueber `applyLang → selectMenuMode(menuSel)`)
und landet im Arena-Football-Hub mit der ersten direkten Karte - kein Modus wird
automatisch betreten. `hubGame` bleibt vor dem Boot `'ro'`, damit genau der normale
„Arena-Kontext betreten"-Zweig von `selectMenuMode` laeuft und `fbHubSel` zuruecksetzt.
**Ring Out bleibt vollstaendig**: ein Klick auf seine Karte fuehrt in den zuletzt gewaehlten
RingOut-Modus (`roSel`), Modi, Raeume, Online und Rejoin sind unveraendert. Der
Wiedereintritt ist autoritativ: `attemptRejoin`/`joinFertig` bauen Spiel und Variante aus
der Raumkonfiguration und fragen weder `menuSel` noch den Hub.
...
Freigabestands. Classic, Speed und Timed FFA erscheinen online **nicht**; Tactical 1v1 und
Team 2v2 sind seit 2026-09-21 freigegeben.
...
### Arena Football — Modushierarchie H02 (2026-09-09, freigegeben)

**ONLINE ist kein Modus.** Es ist eine Spielumgebung und steht deshalb am ENDE jedes
Pfads, nicht daneben. Die frühere flache Liste (Classic · Tactical · Team2v2 ·
Elimination · Online) ist ersetzt.

**Vier Kategorien.** Die Football-Modusleiste des Hubs trägt `cardFb1v1`, `cardFb2v2`,
`cardFbFfa` und `cardFbBot`. Der CTA startet keine Partie mehr, sondern öffnet den
ersten Unterschritt der gewählten Kategorie.

| Kategorie | Weg |
|---|---|
| **1V1** | Classic 1v1 oder Tactical 1v1 → LOCAL / ONLINE |
| **2V2** | Team 2v2 (einzige Regel, keine leere Zwischenebene) → LOCAL / ONLINE |
| **FFA** | Lives oder Timed FFA → 3 / 4 / 5 Spieler → LOCAL / ONLINE |
| **BOT TRAINING** | vorbereitet, *Coming Soon*, kein Startweg |

**Ein Navigationsschirm.** `#fbNavOv` rendert alle Unterschritte aus der Tabelle `FB_NAV`
in derselben bildgeführten Kartensprache wie der Hub. Er enthält **keine**
Spielregeln: jeder Weg endet auf einem BESTEHENDEN Startknopf (`fbClassicBtn`,
`fbTacticalBtn`, `fbTeam2Btn`, `fbElimN{3|4|5}` mit `fbLivesBtn` bzw. `fbTimedBtn`) oder
auf dem bestehenden Onlineeinstieg (`fbOnlineEnter()`). Ein Schrittstapel
(`fbNavStapel`) macht ZURÜCK deterministisch — **genau eine Ebene**, ohne Sprung in
Altmenüs; auch die Classic-Regelwahl und die Elimination-Einrichtung kehren dorthin
zurück, wenn man von dort kam.

**Ehrliches Gatter.** Was online geht, sagt allein `FB_ONLINE_MODES` über
`fbModeReleased()`. Eine nicht freigegebene ONLINE-Karte ist sichtbar, aber abgeblendet,
`disabled` und mit *NOT AVAILABLE YET* beschriftet — kein Klick, kein Raum, kein halber
Start. Zusätzlich prüft der Onlineweg vor dem Einstieg `TUNE` und `r3dActive`.
...
**Darstellung.** Die D1v2-Bildkartensprache gilt auch in den Untermenüs; es werden
ausschliesslich die freigegebenen Kartenbilder wiederverwendet, keine neuen Assets.
Mobil ist der Schirm 348 px breit mit 146 px breiten Karten, auf dem Desktop
(`min-width:900px`) 720 px mit größeren Karten und Schriften — ein Modal, kein
Vollbild. Alle sichtbaren Texte liegen in EN/DE/TR; im gesamten Dokument hat jeder
`T()`-Aufruf einen hinterlegten Eintrag, es erscheint kein interner Schlüssel mehr in
der Oberfläche.

**Nicht berührt.** Gameplay, Physik, kanonische Arena B, Spawns, Tore, Scoring,
Match-Renderer, Match-Kamera, V9-Kern, Online-Protokoll, Firebase Rules, Raumschema und
RingOut. H02 ist Informationsarchitektur, Navigation und Sichtbarkeit.

### RingOut — Rescue Wall (RW1, lokal/Bot, 2026-09-17)

Reaktive Schutzwand im Bot-Training. Waehrend die Kugeln rollen (`phase==='sim'`) tippt der
Spieler in den aeusseren Ring — 62 bis 145 Prozent des **aktuellen** Radius `R`, die Zone
schrumpft also mit jeder Collapse-Stufe mit. Der Tap rastet ueber den Winkel auf eines von
**zwoelf 30-Grad-Segmenten** (`barrierSegAt`).

| Merkmal | Wert |
|---|---|
| Verfuegbar | lokal, `mode==='bot'`; nie online, nie in Arena Football |
| Einsaetze | 3 je Seat und Match, hoechstens 1 je Zug |
| Nachfuellen | nur neues Match/Rematch — nie neue Runde, nie Collapse-Stufe |
| Lebensdauer | genau ein Zug |
| Wirkung | haelt ausschliesslich die Kugeln ihres Besitzers |
| Restitution | 0,55 (eigener Materialwert, nicht die Kugel-Kugel-Restitution) |
| Zone | 0,62 R bis 1,45 R |
| Asset | `assets/temporary_barrier/export/temporary_barrier_polish2.glb` (1,76 MiB) |

**Reihenfolge im Sub-Step** (`stepSim`): Zeitplan aktivieren -> Bahnstart sichern ->
Integration und Kugelkontakte -> **Wandkollision** -> Ring-Out-Auswertung -> Tickzaehler.
Die Wand haelt damit VOR der Grenzpruefung; eine gehaltene Kugel zaehlt nie als
Ueberschreitung. Alle Einhaengepunkte sind `typeof`-abgesichert wie `settleCollapse` —
ohne gesetzte Wand sind sie wirkungslos, weshalb die Golden-Physik bitgleich bleibt.

**Online ist nicht Teil von RW1.** `rw`-Slot, v4-Turn-Autoritaet, Einsatz-Ableitung aus der
Historie und Firebase Rules sind bewusst nicht portiert (RW2). Der Wandblock liegt zwischen
den Markern `==RESCUE-WALL-START==` und `==RESCUE-WALL-END==` **hinter** `stepSim` und damit
ausserhalb des Football-Schnittfensters, das mehrere Football-Suiten als Ganzes extrahieren.
Seine Debugschalter liest er ueber einen eigenen Parser (`barrierParam`), weil
`test_football_flow` ab der Football-Physik keinen Suchparameter-Leser mehr zulaesst.
...
### Arena-Football · ELITE BOT (lokales 1v1, 2026-09-23)

**Weg:** Arena Football → **TRAINING** → **VS ELITE BOT**. Die TRAINING-Karte fuehrt — wie 1 VS 1 — in
eine zweite Auswahlebene (`#fbTrainOv`, Register `FB_HUB_TRAINING`); heute steht dort genau ein Eintrag.
Gestartet wird ueber denselben `startFootball('classic', FOOTBALL_RULES_FIRST3, true)` wie jedes lokale
Match — **kein zweiter Startpfad**. Das dritte Argument ist der einzige Ort, an dem `fbBotModus` gesetzt
wird; jeder andere Einstieg ruft ohne es auf und loescht den Modus damit automatisch.

**Vorausberechnung auf dem echten Motor.** Es gibt keine zweite Physik, die auseinanderlaufen koennte:
`fbBotSim(kopie, schritte)` tauscht `balls` gegen eine Kopie und ruft `stepSim()`. Damit das nichts
anrichtet, laeuft es unter dem Schalter `fbVorausTiefe` / `fbVorausAn()`, der unmittelbar vor `stepSim`
steht:
- der Rueckmeldungsblock in `stepSim` (Klang, Partikel, Bildimpuls, Musik) ist mit `if(!fbVorausAn())`
  umschlossen — die Physik darueber bleibt Zeile fuer Zeile dieselbe,
- `footballTryGoal` **meldet** ein Tor nur (`fbVorausTor`) und wertet es nicht,
- `fbBotSim` sichert `balls`, `fbGoalState`, `fbGoalTick`, `footballWinner`, `phase`, `outBall` und den
  Stand und stellt alles im `finally` zurueck.

`stepSim()` hat dadurch einen **dritten** Aufrufer neben dem festen Schritt und der Online-Rehydrierung.
Er treibt keine Spielzeit: er rechnet auf einer Kopie. Die Fixed-Timestep-Suite benennt ihn ausdruecklich.

**Fairness ist gebaut, nicht versprochen.** Der Planer erhaelt ausschliesslich `fbBotSicht(wer)` —
Koerperkopien, Stand, Runde, eigener und gegnerischer Sitz. Zugvektor, Richtung, Kraft und ein nicht
aufgedeckter Commit des Menschen stehen dort nicht, und der gesamte Planer nennt `commitAim`, `aimSet`
und `curAimer` an keiner Stelle. Die Suite prueft beides — die Quelle **und** das Verhalten (drei
verschiedene verdeckte Eingaben, derselbe Zug).

**Suche (alle Grenzen zentral in `FB_BOT`).** Grob werden alle Kandidaten mit je einer Simulation
bewertet: 11 Winkel um die Ballrichtung (±0.62 rad) × 4 Kraftstufen, dazu 6 Bandenwinkel und ein weicher
Zug — 51 Stueck. Die besten 6 laufen fein gegen 5 plausible **Gegnerzuege aus derselben oeffentlichen
Ausgangslage** (nie gegen den tatsaechlichen verdeckten Zug); gemischt wird 62 % schlechtester Fall,
38 % Mittel. Zusammen 81 Simulationen je Planung. Bewertet werden Tor/Gegentor, Ballweg zu beiden
Tormaeulern, Naehe, freie Bahn und Deckung; der Spielstand verschiebt die Risikobereitschaft.

**Zeitverhalten.** Der Auftrag entsteht beim **Rundenbeginn** — im Settle von `stepSim`, mit
`!fbVorausAn()` gegen Rekursion gesichert, und zusaetzlich in `startRound` fuer Matchbeginn und Anstoss.
Gerechnet wird danach **scheibenweise** im Bildlauf (`fbBotScheibe()`, `FB_BOT.scheibeMs` = 6 ms je Bild,
rund 9 Bilder), waehrend der Mensch zielt. `fbBotAuftrag` / `fbBotArbeiten` teilen die Arbeit in gleich
grosse Stuecke; Reihenfolge und Zufallsziehung sind dieselben wie am Stueck, das Ergebnis ist bitgleich.
`fbBotSuche()` (am Stueck) bleibt der Weg fuer Tests und Messstand; `fbBotFertig()` rechnet den Rest zu
Ende, falls zu frueh losgelassen wird.

**Was `scheibeMs` zusichert - und was nicht.** Es ist eine UNTERGRENZE, keine Obergrenze: die
Budgetpruefung steht NACH einer Arbeitseinheit, damit je Bild mindestens eine Simulation fertig wird -
sonst koennte die Suche auf einem langsamen Geraet nie enden. Der Schlechtfall einer Scheibe ist deshalb
„Budget + eine Arbeitseinheit". Gemessen im Browser unter SwiftShader (Software-Renderer, also deutlich
langsamer als echte Hardware), drei Laeufe, 21-38 Scheiben je Lauf und Geraet:

| Lauf | Desktop max | Handy max |
|---|---|---|
| 1 | 17,50 ms | 10,20 ms |
| 2 | 10,50 ms | 10,40 ms |
| 3 | 10,40 ms | 12,50 ms |

Schlechtester beobachteter Wert 17,50 ms (Desktop, Maschine unter Last); p50 lag durchweg bei 6,1-6,7 ms.
Rund 20 Scheiben je Lauf ueberziehen das Budget leicht - das ist der Entwurf, nicht ein Fehler. Um MEHR
als eine Arbeitseinheit ueberzog in keinem Lauf eine einzige Scheibe, und jede Scheibe rechnete mindestens
ein Stueck (kein Leerlauf). Die teuerste einzelne Arbeitseinheit lag bei 10,3-10,5 ms. Auf echter Hardware
kostet eine Simulation rund 1 ms (81 Stueck in ~80 ms), der Schlechtfall laege dort bei rund 7 ms.

Die Bildabstaende derselben Laeufe (Handy p95 ~360 ms, Desktop p95 ~1480 ms) messen NICHT den Planer,
sondern den Software-Renderer - jedes einzelne Bild kostet dort ein Vielfaches der laengsten Scheibe.
...
**Pfosteneffekt entfernt (2026-09-23).** Die weissen Funken im Torbereich stammten von `fbFeelPost`
(Blitz, Sternglanz, Splitter und Ring bei jedem Pfosten-/Pfostenfusstreffer, beim rollenden Ball in
Serie). Die Funktion und ihre zwei Aufrufer (gefegter Pfostentest in `stepSim`,
`footballResolvePost`) sind entfernt; der Pfostenklang `fbSfxImpact(...,'post')` bleibt. Damit
erzeugen Bande und Pfosten keine fx3-Effekte mehr; es bleiben Ballkontakt, Figurenkontakt und Abschuss.

**Ring Out vorerst nicht im Angebot (2026-09-25).** Produktentscheidung: Arena Football ist das
Hauptspiel, RINGOUT bleibt die Marke. `RINGOUT_SPIELBAR` ist die EINE Stelle (auch `?dev=1` oeffnet sie nicht; nach einer Ring-Out-Partie sperrt `rematchErlaubt()` die Revanche; die QA-Pruefstaende schalten Ring Out nur im Adapter frei):
`body.roAus` blendet die Spielwahl (`.roHub`) aus, `selectMenuMode` faellt auf Arena Football zurueck,
der CTA beginnt keinen Ring-Out-Weg, `createRoom` legt keinen Ring-Out-Raum an und `joinRoom` nimmt
dort niemanden neu auf - NACH der Eigensitz-Uebernahme, damit laufende Partien erreichbar bleiben.
Der Ring-Out-Code, seine Rules und Online-Fassungen (8/12/13) bleiben unveraendert. Wieder
einschalten: `RINGOUT_SPIELBAR=true`.

**Planung klingt nicht (2026-09-24).** `fbSfxImpact` ist die eine Stelle fuer Football-Kontaktklaenge
und schweigt unter `fbVorausAn()` - die Vorausberechnung des Bots faehrt den echten `stepSim` auf
Kopien, deren Banden- und Pfostenkontakte sonst hoerbar waren. Jeder kuenftige Kontaktklang ueber
diese Schicht ist damit automatisch planungsstumm.

**Szene vor dem Spiel (2026-09-24).** Jeder Weg in ein Football-Match sichert vorher die 3D-Szene
(`await r3dSichern(true)`), auch die Karte TRAINING (`fbTrainOeffnen`). `showGame` laedt sie nur
nach und zeigt das Spiel sofort - wer es ohne vorheriges Warten aufruft, zeigt die 2D-Arena.
...
Stelle wie beim Matchstart), sonst waere der Plan des vorigen Matches noch gueltig.

**Was der Spieler sieht.** Kein Uebergabeschirm (`fbOhneUebergabe()` — gemeinsames Fenster **oder** ein
einziger Mensch am Geraet), die Gegnerkarte im HUD traegt `ELITE BOT` statt `ROT`, und das Namensschild
ueber der roten Figur ebenso (`pName`). Alles andere bleibt Classic 1v1: Aufstellung, Physik, Tore,
erster bis 3, Anstoss nach dem Tor, Rematch.

**Namensgebung:** Der Schalter heisst `fbVoraus*`, nicht `fbPlan*` — `fbPlan*` ist im Produktivcode
gesperrt (entfernter Tactical-Dual-Prototyp, von einer Suite bewacht).
...
gezogen). Die Spiegelprobe `bank.js --spiegel` dreht ausschliesslich den roten Zug um eps und erhaelt
auf jeder Stufe einen glatten Durchmarsch statt eines Splits (0 -> 0:16, 1e-9 -> 0:16, 1e-6 -> 16:0,
1e-3 -> 0:16, 1e-1 -> 16:0). Der Vorlauf im Messstand behebt deshalb keinen Bias, sondern erzeugt
verschiedene Partien: eine Seite spielt einen zufaelligen legalen Zug, die andere setzt in diesem einen
Vorlauf aus, beides durch dieselbe gleichzeitige Aufloesung; die eroeffnende Seite WECHSELT je Spiel
(`i % 2`). Das steht ausschliesslich in `artifacts/elitebot-01/bank.js` - das Produkt kennt weder
Zugwechsel noch eine geaenderte Kollisionsaufloesung. Seitenabhaengigkeit gemessen ueber 90 Matches:
als Blau 44/45, als Rot 45/45 - der Unterschied ist je Gegner eine einzige Partie.

Der
Browser-Nachweis (`artifacts/elitebot-01/sicht.js`) faehrt den Produktweg auf Desktop und Handy.
**Offen:** Schwierigkeit und Spielgefuehl gegen Menschen — dazu sagen diese Zahlen nichts.
...
### Die Zustandszeile: zwei feste Kinder, keine Uebersetzung (2026-09-22)

- **Warum.** Gemeldet wurde ein Sprachflackern in Tactical 4-Ball: derselbe Moment zeigte
  abwechselnd "YOUR TURN · 7.6" und "IHRE REIHE · 7,5". "IHRE REIHE" steht in KEINER der drei
  Sprachtabellen, und ein Dezimalkomma schreibt das Produkt nirgends (immer `toFixed(1)`).
  Der deutsche Text kam also von der SEITENUEBERSETZUNG DES BROWSERS. Sichtbar machte ihn der
  Anstrich: `fbClockPaint` laeuft in jedem Bild, und die Zeile wurde dabei geleert und neu
  zusammengesetzt - Beschriftung und Ziffern als frische Knoten. Jeder neue Textknoten ist
  fuer eine Uebersetzung ein neuer, asynchroner Auftrag; bei zehn Neuaufbauten je Sekunde
  sprang das Ergebnis zwischen Produktsprache und Uebersetzung.
- **Aufbau.** `#fbState` traegt zwei FESTE Kinder: `#fbStateLbl` (Beschriftung, wird
  vorgelesen) und `#fbStateNum` (`aria-hidden`, die laufende Zahl). Der Anstrich beschreibt
  sie und erzeugt nichts; jeder der beiden wird nur bei echter Aenderung geschrieben. Waehrend
  eines Countdowns aendert sich ausschliesslich die Zahl. Die Ansagezusage bleibt unveraendert:
  angesagt wird die Beschriftung, nie die laufende Zahl.
- **Uebersetzung.** Genau diese eine Zeile traegt `translate="no"`. Ihr Inhalt ist
  Produktsprache aus der eigenen Tabelle (Einstellungen: English/Deutsch/Tuerkce). Die Seite
  bleibt sonst vollstaendig uebersetzbar - eine globale Abschaltung waere eine Produktentscheidung
  und ist ausdruecklich NICHT getroffen worden.
- **Nachweis.** `tools/test_football_decision_hud.js` Abschnitt 6B misst ueber 40 Bilder:
  vorher 80 erzeugte Knoten und die festen Ziffern 0-mal beschrieben, nachher 0 erzeugte Knoten
  und die Beschriftung hoechstens einmal gesetzt. Derselbe Prueftstand gegen den alten Anstrich:
  8 Fehlschlaege. Im echten Browser (`artifacts/tactical4_01/lauf.js`) 16 Abtastungen ueber
  2,4 s: acht Countdown-Staende, EINE Beschriftung, derselbe DOM-Knoten, kein Dezimalkomma.
- **Ring Out** ist nicht betroffen: Zeile und Anstrich haengen am Football-Kontext
  (`#game.fb .fbstate`); ausserhalb schreibt der Anstrich nach dem ersten Bild nichts mehr.
- **Offen (Produktentscheidung).** Die In-Game-Ebene ist absichtlich deutsch verdrahtet
  ('Runde N', 'zielt…', BLAU/ROT, und ueber `I18N.de.*` der Football-Untertitel "ERSTER BIS 3"),
  waehrend die Zugzeile der gewaehlten Sprache folgt - im HUD steht deshalb weiter ein deutscher
  Untertitel neben einer englischen Zugzeile. Dokumentiert in `tools/test_football_shell.js`
  Abschnitt C; eine Vereinheitlichung waere eine eigene Entscheidung.

### Namensschilder ueber den Figuren (beide Spiele, 2026-09-20)
- **Eine Zeichnung, zwei Sichtbarkeitsregeln.** Der Name steht in `drawBall()` Abschnitt 6 — gemalter Canvas-Text auf dem vorhandenen Overlay, in 2D wie in 3D ueber `OP(b.x,b.y,0)`. Schrift `800 <fs>px system-ui` mit `fs = max(12, BR*.66*s)`, Lage `y = q.y − BR*1.55*s`, dunkle Kontur (`lineWidth = fs*.28`), Farbe `ncol(colorSlot(b.owner))` (Football: `FB_NAME_COL`). Zu lange Namen werden **nur fuer die Anzeige** auf `BR*3.6*s` Breite gekuerzt (`…`); der gespeicherte Name bleibt unberuehrt, die Laengengrenze setzt weiterhin `sanitizeName()`/`NAME_MAX`.
- **Wer traegt ein Schild (seit 2026-09-22):** die Frage lautet immer *wem gehoert diese Figur?* — und das Schild steht nur dort, wo sie offen ist. **Ohne** Schild: der neutrale Ball (gehoert keinem Sitz), das Menue-Vorspiel (dort laeuft kein Match) und **Tactical 1v1 / Tactical 4-Ball** (`fbTactical()`, beide Varianten): dort stehen genau ZWEI Menschen gegeneinander, die Farbe sagt bereits alles, und die Namen stehen oben im Spielstand (`updateHud` → `#n0`/`#n1`). **Mit** Schild: **Team 2v2** (vier Menschen, vier Figuren) und **FFA/Elimination** (eine Figur je Startspieler) — dort traegt erst der Name die Zuordnung.
- **`nameLabelOn(b)` ist die einzige Stelle, an der ueber Sichtbarkeit entschieden wird.** Ring Out: sichtbar ausser in der Simulation (unveraendert). Arena Football: **durchgehend, auch waehrend der Physik** — dort schiessen alle Sitze gleichzeitig, und erst der Name sagt waehrend des Zuges, wessen Figur wohin laeuft. Ohne Schild bleiben in Football: der **neutrale Ball** (`FOOTBALL_NEUTRAL_OWNER === 5`, gehoert keinem Sitz), das **Menue-Vorspiel** (`menuVisible`) und die Variante **Tactical** (eine Identitaet fuehrt dort zwei Figuren).
- **Namensquelle ist `pName()` → `nameForSeat()` → `players/<seat>/name`.** `b.owner` **ist** der Sitz; es gibt **keine zweite Zuordnung**, die veralten koennte. Jedes Bild liest neu, und `playersRoster` wird bei Austritt geleert sowie bei Beitritt/Rueckkehr ersetzt — ein Name aus einem frueheren Raum kann deshalb nicht stehen bleiben. Eine passive (getrennte) Figur bleibt koerperlich im Feld und behaelt ihren Namen.
- **Warum Canvas und nicht DOM:** gemalte Pixel koennen keinen Zeiger abfangen (das Zielen am Telefon bleibt unberuehrt), es entsteht kein Knoten je Teilnehmer, nichts kann ueber den Rand der Flaeche ragen, und es gibt keinen Lebenszyklus zum Aufraeumen — das Schild lebt exakt so lange wie die Kugel, die es traegt. Je Bild fallen nur die Textmasse an.
- Pruefstand: `tools/test_football_names.js` (92) fuehrt das **echte** `drawBall` gegen einen aufzeichnenden Zeichenkontext; Laufzeitbelege und Bilder in `artifacts/namen01/`.
...
Ringout/
  ...
  assets/hub/modes/  # elf Moduskarten-Bilder (5 RingOut, 6 Arena Football inkl. football_tactical4)
