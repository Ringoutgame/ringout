...
**Zuletzt aktualisiert:** 2026-07-31 (Dawn ist der einzige öffentliche Environment-Look — Day aus Settings und Runtime entfernt; Branch `feature/dawn-only-environment`, noch nicht committet)
...
**Environment-Look: Dawn only (2026-07-31):** RingOut hat **einen** öffentlichen Himmel — Dawn (`assets/hdri/qwantani_sunset_puresky_2k.hdr`, Profilwerte unverändert seit der Freigabe in 94d17fc). Die frühere Dawn/Day-Auswahl ist aus den Settings entfernt; das Modal zeigt nur noch **Sprache · Vibration · OK** (eine Auswahl mit einer Option ist keine Auswahl). `ENVIRONMENT_PROFILES` behält bewusst die Map-Struktur — ein späteres Night-Profil ist ein zusätzlicher Datensatz, kein Codeumbau. **Kein Protocol-Bump** (rein visuell/UI). Der versionierte Storage-Key `ringout_environment_profile_v1` bleibt als Migrationspfad erhalten: `readEnvProfile()` normalisiert Altwerte (`day`, `dawnGolden`) und **korrigiert den Storage einmalig** auf `dawn` — noch vor `initR3D()`, also ohne Flash und ohne schwarzen Himmel. Die gemeinsame HDRI-Fallback-Kette (`kloofendal` → `qwantani_puresky` → `qwantani_sunset`) ist unverändert; **`assets/hdri/qwantani_puresky_2k.hdr` bleibt bewusst im Repo** als Sicherung, obwohl kein Profil es mehr primär lädt. Mit dem Wegfall der Auswahl entfiel auch die Laufzeit-Umschaltung (`setEnvProfile`/`flushEnvProfile`/`envPending`) — der Look wird einmal beim Start geladen; die Git-Historie hält Day-Datensatz und Umschalt-API für ein künftiges Night-Profil vor.
...
- **Online-Timer 60s/7s + Online-Collapse (Branch `feature/online-timer-60s-7s`, lokal fertig 2026-07-28 — Commit/Rules-Deploy ausstehend):** serverautoritative Match-Uhr ohne Backend. Jeder Turn erhält einen write-once Phasenstempel `g/<gen>/t/<turn>/s = serverTimestamp()` (Rules: `=== now`, nur aktuelles Gen, nur `playing`; jeder Client stempelt beim Armen, das Write-once-Race entscheidet, der tatsächliche Serverwert wird per set→get-Adopt deterministisch übernommen und blockiert bis dahin das Reveal). Jeder Zug trägt sein `ts = serverTimestamp()` **atomar in derselben Write-once-Transaction** wie `idx/dx/dy/sp` (`applyLocally:false`; das SDK löst den Sentinel serverseitig VOR der Rules-Prüfung auf — per Browser-Probe verifiziert). Rules: `ts` Pflicht genau bei gestempeltem Turn, sonst verboten (v3-Bestandsräume unverändert, `ONLINE_PROTOCOL_VERSION` bleibt 3); Deadline-Gate `s+7000` lässt danach nur No-Shots zu. Aktive Zeit = `min(7000, ts−s)` pro Turn (nur Aim zählt; Physik/Reveal/Result nie), letztes Fenster auf Match-Restzeit gekappt; **Collapse-Turn** = erster vollständiger Turn mit kumuliert ≥ 60s (`onlineCollapseTurn`, pure Funktion über gespeicherte Stempel → alle Clients identisch, listener-latenz-unabhängig). Anwendung exakt einmal pro Gen (`onCollapsedGen`-Latch) an der Rundengrenze (Settlement-Hook in `stepSim` / `afterResult`), Radius wie lokaler Collapse (`COLLAPSE_RADIUS_FACTOR`); `fastForwardMatch` liest dieselben Stempel → Replay/Rehydration reproduziert dieselbe Grenze bit-identisch. Timeouts werden nach der gemeinsamen Deadline nur ANGEFORDERT (No-Shot durch dieselbe atomare Write-once-Transaction, parallele Anfragen idempotent, bestätigte Züge unantastbar). Lokale Browserzeit dient nur HUD + Anfrage-Timing (`.info/serverTimeOffset`), nie der Collapse-Entscheidung. Bestandsgrenze (v3 ohne Auth, unverändert): verfrühter fremder No-Shot ist von legitimem Stehenbleiben nicht unterscheidbar (Leave-Sentinel-Pfad). Tests: Runner 16/16 (Rules 195, Collapse 313, Reconnect-E2E 88), Rules zusätzlich 25/25 gegen den echten RTDB-Emulator.
...
### Ring-Collapse Cracked-Segmente — Stand VISUELL FREIGEGEBEN (2026-07-26)
- **Verbindlicher Stand:** `assets/ring_collapse/validation/six_segment_rollout/ring_collapse_six_segment_simplified.glb` — 12 Roots (Segment 01–06 je `*_Intact` + `*_Cracked_BALANCED`). Alle sechs Segmente besitzen damit Cracked-Varianten; Segment 06 ist als **Dark-Facet-Removal-Stand** freigegeben und eingefroren.
- **Reproduzierbar ab dem committeten 10-Root-Basisstand — kein Neubau von null:** Committet sind der freigegebene Final-GLB, der freigegebene 10-Root-Stand `seam_corridor_final_fix.glb`, der Produktions-Blend, die 9 buildnotwendigen Quelltexturen aus `source/textures/`, die fünf Pair-3-Texturen, die Builder-/Validator-/Graft-Skripte, der vendorte three.js-r170-Baum inkl. Lizenz/Manifest, der Final-only-Browser-Smoke sowie Doku und kuratierte Freigabenachweise. Alle Gate-, Rollout- und Graft-Zwischen-GLBs (`seg36_gate.glb`, `seg36_rollout.glb`, `seg06_simplified.glb`, `seg06_simplified_rollout.glb`) sind **reine Build-Artefakte** — sie werden von den Skripten deterministisch neu erzeugt und sind bewusst **nicht** committet.
- **Erzeugungskette (deterministisch, fünf Schritte, in dieser Reihenfolge):**
  1. `tools/validate_ring_collapse_seg36_gate.py` — liest Blend read-only → `seg36_gate/ring_collapse_seg36_gate.glb` (Segment 03 + 06 roh) + bäckt den kompletten Pair-3-Atlas frisch (beide Hälften)
  2. `tools/validate_ring_collapse_seg36_rollout.py` — liest `seam_corridor_final_fix.glb` + Ausgabe von Schritt 1 → `seg36_rollout/ring_collapse_seg36_rollout.glb` (Gold-Taper + Seam-Harmonisierung; liefert das harmonisierte Segment 03)
  3. `tools/validate_ring_collapse_seg06_simplified_gate.py` — liest Blend read-only + lädt den Atlas aus Schritt 1 (selber Ordner) und bäckt nur die Segment-06-Hälfte neu → `seg36_gate/ring_collapse_seg06_simplified.glb`
  4. `tools/validate_ring_collapse_seg06_simplified_rollout.py` — liest `seam_corridor_final_fix.glb` + Ausgabe von Schritt 1 (Nachbar-Kontext) + Ausgabe von Schritt 3 → `seg36_rollout/ring_collapse_seg06_simplified_rollout.glb`
  5. `tools/ring_collapse/graft_simplified.js` — liest `seam_corridor_final_fix.glb` + Ausgabe von Schritt 2 (Segment 03) + Ausgabe von Schritt 4 (Segment 06) → byte-erhaltender Graft + finaler Radialfeld-Pass + Census-Snap → der verbindliche 12-Root-Stand

  Prüfung: `tools/validate_ring_collapse_six_segment_rollout.py` — die Defaults zeigen auf den freigegebenen PNG-Stand (Final-GLB + Simplified-Rollout-Pair), es sind keine Env-Variablen nötig; Overrides `SIX_GLB`/`PAIR06_GLB` bleiben verfügbar, die KTX2-Prüfung ist erst nach dem KTX2-Folgetask per `CHECK_KTX2=1` zu aktivieren. Alle Ring-Collapse-Validatoren beenden sich seit dem Codex-Fix mit Exit-Code ≠ 0, sobald eine Assertion fehlschlägt (kein False-Green in CI/Ketten). Dependency-Closure (geschlossen und verifiziert 2026-07-26 durch einen Clean-Checkout-Scratch-Rebuild): jede gelesene Datei ist committet oder Ausgabe eines vorherigen Schritts. Der Produktions-Blend verlinkt **insgesamt 15 Texturen**; **9 davon sind buildnotwendige Source-Dependencies und Teil des Commits** (`rc_body_basecolor/normal/orm`, `rc_fracture_basecolor/normal/orm`, `rc_top_basecolor_clean`, `rc_top_normal`, `rc_top_orm_clean` in `source/textures/`). Ein Checkout, der nur den Commit enthält, lief alle sechs Schritte grün (29/39/17/29 Assertions, Graft, Final-Validator 14 PASS + 1 dokumentierter KTX2-Skip) sowie den Browser-Smoke mit Exit 0. Der Rebuild ist **formidentisch, aber nicht byte-identisch**: die 10 geschützten Roots entstehen identisch, die beiden neu gebauten Roots haben dieselben eindeutigen Vertex-Positionen und Dreieckszahlen (9094 / 9465), unterscheiden sich aber in je einem Attribut-Split-Duplikat plus Vertex-Reihenfolge; dazu kommen drei neu gebackene Atlas-PNGs. Der committete GLB bleibt der verbindliche Stand. Die übrigen 6 (`rc_top_{basecolor,normal,orm}_stage1/stage2`) dienen ausschließlich der GUI-/Preview-Überblendung im Blend-Material `M_ArenaTop`, werden vom Builder regeneriert (er erzeugt alle 15 Texturdateien selbst) und sind bewusst **nicht** committet — **die vollständige Build-Kette startet und läuft grün ohne sie**. Funktionsgleichheit des canonical Graft-Skripts gegen die vorherige Artifact-Fassung per Byte-Vergleich der Ausgabe verifiziert (identischer MD5).
- **Byte-Erhalt als Architekturprinzip:** Der freigegebene Bin des 10-Root-Basisstands ist Verbatim-Präfix des neuen Bins, alle JSON-Arrays (bufferViews/accessors/materials/textures/images/meshes/nodes) sind Präfixe → die 10 zuvor freigegebenen Roots bleiben byte-identisch, Segment 03 ebenfalls. Neue Inhalte werden ausschließlich angehängt (51 BufferViews, 48 Accessors, 3 Bilder, 1 Material `M_Fracture_Cracked_C`).
- **Verbindliche Messwerte:** 1 Insel für Segment 06 · Thin-Groups 0 · Weld-Open-Edges 94 · Intact-Pixeldiff exakt 0 · Silhouette = intact (max z 0.0620) · Gate 17/17 · Rollout 29/29 · Validator 14/14. Die Browser-Ergebnisse des Freigabelaufs (0 Fehler / 0 externe Requests, Before/After-Pixeldiffs) sind **archivierte Freigabenachweise** (Captures + `simplified_capture_debug.json`); **reproduzierbar aus dem Commit** ist der Browser-Beleg über den canonical Final-only-Smoke-Test `node tools/ring_collapse/browser_smoke/run_smoke.js` (lädt nur den freigegebenen Final-GLB über den vendorten three.js-r170-Baum; Exit 0 = 12 Roots, 1 Frame gerendert, 0 Konsolen-/Page-Fehler, 0 externe Requests).
- **Cleanup-Architektur (nicht offensichtlich, daher dokumentiert):** Dünne Deckplatten im Risskanal werden **nicht** mehr weggeschnitten (die früheren Consume-Dishes hinterließen sichtbare facettierte Innenflächen), sondern von unten verdickt — nur verdeckte Gegenhäute bewegen sich, Verts nahe der Plateau-Oberkante (`TOP_Z`) nie. Ergänzend legt ein Census-Snap Vertex-Paare ≤0,5 mm zusammen, die der Gold-Taper auseinandergezogen hatte (reine Positionsangleichung; Normalen und Topologie unverändert).
- **Offen:** Der Pair-3-Atlas (`validation/seg36_gate/textures/seg36_frac_*.png`) liegt weiterhin als PNG vor — **KTX2-/HQ-Finalisierung ist ein separater Folgetask**.
- **Nicht Teil der Freigabe:** verworfene Zwischenstände (`preview1`, `final1`, `micro_fix`, ältere Segment-06-GLBs sowie deren Builder-/Graft-Skripte) — sie existieren nur lokal als Historie.

### Ring-Collapse-Visual — Produktionsintegration (2026-07-28, Branch `feature/ring-collapse-visual`, unkommittiert)
- **Reiner Render-Adapter** (`colv*` in `initR3D`, `index.html`) über der unveränderten Collapse-Wahrheit — `R`, `collapseState` und `onCollapsedGen` bleiben die alleinige Quelle; keine neue Gameplay-/Online-/Timer-Logik, Physik/Lockstep byte-unangetastet. Die Stufe wird pro Frame ABGELEITET (idempotent), nie eigenständig entschieden: Stufe 2 nutzt exakt das `shrinkFloor`-Prädikat, Stufe 1 das 10s-Warnfenster (lokal `collapseRemainMs`, online `onlineRemainMs`).
- **Verbindliches Modell (finale Fassung):** Im Uhr-Match wird beim Matchstart (maskiert durch den Menü→Spiel-Übergang) das äußere Ringband getauscht: `Walkway` + V1-Drum-Deko (`Cornice*`/`GoldCornice*`/`GoldCapital`/`WallRing`/`Pillar*`) aus, stattdessen die zwölf Segment-Roots (6 intakt + 6 gerissen, paarweise) als Vollkörper-Band auf Walkway-Höhe (`COLV_SEG_LIFT` = +0.125, Top exakt .165); Materialien werden beim Laden ÜBERNOMMEN: `M_ArenaTop*`/`M_ArenaMarble` → Plattform-`M_Marble`, `M_ArenaGold` → Plattform-`M_Gold` (identische Instanzen — kein Farbton-Nachbau), Bruchflächen behalten `M_Fracture*`. Die durchgehenden V1-Goldringe (`GoldStepEdge`/`GoldWalkRing`) und die helle Grenz-Deko (`bGroup`) sind im Uhr-Match komplett aus — Gold liegt auf dem Band nur als segmentgebundene Bögen des Assets (enden an Rissen/Fugen, fallen mit jedem Segment); die schmale Fuge 8.7/8.78 liest sich als Dehnungs-/Bruchlinie. Restkörper wird NIE skaliert/verschoben (kein `colvShrink` mehr) · (1) Warnfenster: `colvCracks(prog)` schaltet die Paare gestaffelt intakt→gerissen (Reihenfolge = Abriss-Shuffle, Schwellen 0/.16/.32/.48/.64/.80 des Fensters) — identische Silhouette, nur echte Risse · (2) Abriss: Segmente kippen/stürzen aus ihrer sichtbaren Ausgangsposition (Tremor → ~8° Abkippen um die Tangentenachse → freier Fall mit Taumeln, ~2,6 s, deterministisch); die breiten Sockelteile unter dem Band (`TierBridge`/`Tier1`/`TierRib*`/`TierStud*`/`GoldTier1`, `colvPed`) stürzen leicht verzögert mit (gerade nach unten) — der Boden kragt danach über dem schmaleren Kelchstumpf aus; nichts verschwindet vorher, kein Snap.
- **Collapse-Audio (2026-07-29, Branch `feature/ring-collapse-sound`, unkommittiert — bild-synchrone Runtime-Fassung, live abgenommen):** ausschließlich echte CC0-Aufnahmen (4 Quellen, s. `assets/sfx/README.md`; 14 onset-korrigierte WAV-Derivate via `tools/sfx/build_ring_collapse_sfx.js` — Auto-Trim: Haupttransient 4 ms nach Samplebeginn, Limit ≤10 ms). **Architektur: Bild und Ton aus derselben Quelle.** Der visuelle Adapter meldet jeden TATSÄCHLICH sichtbar geschalteten Zustandswechsel im selben Frame read-only an `SFX.colvEvent` (einzige Auslöserstelle `colvSnd` in `colvTick`): Warnriss-Erscheinen (gleiche `crackAt`-Wahrheit, `crackMask`-Latch, Bestand bei spätem Einstieg/Frame-Lücke stumm), Hauptbruch (Stufenwechsel `colv.state→2`, exakt einmal, `instant` stumm), Segmentbruch (erster Frame der Ablöse-/Fallbewegung, `fellSnd`-Latch, 0,25-s-Toleranz), Nachbröckeln (einzelne kurze Fragmente `fragc_*`/`fragf_*` NUR bei sichtbarem Materialverlust, Dichte folgt `fragW`, endet vor dem ruhigen Endzustand). Kein `SFX.strain`, kein `SFX.collapse()`, kein `colvSegAudio`-Export, keine Cue-/Timer-Tabelle mehr; Timer-Core ist soundfrei (nur warn/tick-Beeps + `collapseStop`). Zielzeit zentral kalibriert: `colWhen = currentTime + max(0, 1 Frame − outputLatency)`. Vorladen `SFX.colPreload` ab Matchstart (decode fertig vor dem Warnfenster); nicht bereit/GLB-Fehler ⇒ kontrolliert still, nie nachgeholt. Kein Oszillator im Collapse-Pfad, kein synthetischer Fallback. Suite `tools/test_collapse_sfx.js` (111, Kausalität: Sound exakt im Tick des sichtbaren Wechsels, inkl. Prüfung „keine Preview-/Diagnoseschalter mehr im Code"). **Nach Owner-Abnahme bereinigt:** die für die lokale Hörprobe temporären Schalter `?colfast` (Timerverkürzung) und `?sfxdebug` (Diagnose-Overlay + Debug-Hook `setColDiag`/`colDiagFn`) sowie der darauf aufbauende Live-Test `tools/test_collapse_sfx_live.js` sind vollständig entfernt — `MATCH_COLLAPSE_SECONDS` ist wieder eine feste Konstante (60), kein Preview-Code im Produktpfad. `tools/serve_local.js` (generischer lokaler Static-Server, `npm run serve`) bleibt als allgemeines Dev-Tool erhalten.
- **Kanten-Geometrie:** Plattform-Divisor wechselt im Umschalt-Frame von 10.1 auf 8.282 (= 10.1×0.82 — R ist im selben Frame ×0.82 geschrumpft → Plattform-Scale mathematisch stetig, der Restkörper bewegt sich nie). Bewusste Toleranz: sichtbare Restkante 8.7 GLB ≈ 1.05×R — der Spielradius liegt ~eine halbe Kugelbreite innerhalb der sichtbaren Kante (Preis für den nie skalierenden Restkörper); Kugel-Stufen-Lift .165 konstant, nach dem Abriss 0.
- **Lebenszyklus:** Asset (23 MB) lädt lazy beim Matchstart (~60 s Vorlauf vor dem ersten möglichen Einsatz); Ladefehler ⇒ exakt das bisherige Verhalten (uniformes Mitschrumpfen, `colv.load===3`-Fallback). Rehydration/Replay, die direkt im kollabierten Zustand landen (Stufe 0→2), springen ohne Animation in den Endzustand. Menü/neues Match (`Stufe 0`) stellt Walkway, Segment-Ruhelagen und alle Skalierungen vollständig wieder her.
- **Verifikation:** Runner 16/16 (keine Testzahl geändert — Adapter liegt außerhalb der extrahierten Blöcke), headed-Playwright-Smoke Bot-1v1 komplett (Warnband → Abriss → Endzustand → Weiterspiel → Menü-Restore; 0 Spiel-Konsolenfehler), Screenshots in `artifacts/ring_collapse_visual/`. **Umgebungs-Notiz:** headless SwiftShader-Chromium verliert `requestAnimationFrame` beim ersten Launch lokaler Matches (am unveränderten Release-Stand reproduziert; headed/Produktion nicht betroffen) — Browser-Automation lokaler Modi daher headed fahren.

### Two-Stage-Collapse — verbindlicher Vertrag (Owner-Entscheidung 2026-08-02, Branch `feature/two-stage-collapse`)

**Zeitachse (lokal wie online identisch abgeleitet):** zwei Zyklen à exakt `COLLAPSE_CYCLE_SECONDS = 30` Sekunden. 0–20 s normal, 20–30 s Warnphase 1, bei 30 s Collapse 1; 30–50 s normaler zweiter Zyklus, 50–60 s Warnphase 2, bei 60 s Collapse 2 (terminal). `COLLAPSE_WARNING_SECONDS` bleibt 10, `COLLAPSE_STAGE_COUNT = 2`. Kein dritter Collapse. Das HUD zeigt immer die Restzeit des laufenden 30-s-Zyklus.

**Zustandsmodell:** lokal `collapseStage` = 0/1/2 (Anzahl vollzogener Collapses); online `onCollapseCount` = 0/1/2 je Generation (ersetzt den bisherigen booleschen `onCollapsedGen`-Latch). Radius: Collapse 1 → `R × 0.82` (485 → 397,7), Collapse 2 → erneut `× 0.82` (≈ 326,1). Nach Collapse 1 werden ausschließlich Timer- und Warn-/Sound-Latches für Zyklus 2 zurückgesetzt — Radius, Score, Spielerzustand und Matchgeneration bleiben unangetastet; `collapseState` wird für Zyklus 2 reaktiviert. Nach Collapse 2 gilt exakt das bisherige terminale Verhalten des einzigen Collapses.

**Online-Ableitung (keine neuen DB-Felder, keine neuen Timer-Writes):** Stufen ausschließlich aus den vorhandenen Server-Stamps (`g/<gen>/t/<turn>/s` + `ts`). Erste Schwelle: kumulierte Planungszeit ≥ 30 000 ms, zweite Schwelle: ≥ 60 000 ms — jeweils der erste vollständig abgeschlossene Turn, der die Schwelle erreicht, ist der Collapse-Turn der Stufe. Late Join und Reconnect rekonstruieren Stufe, Radius, Warnzustand und Visualzustand vollständig aus denselben Stamps; Rematch (neue Generation) neutralisiert beide Stufen. `ONLINE_PROTOCOL_VERSION = 4` verhindert gemischte alte/neue Clients (Rules-`v === 4`, Publish separat nach Review).

**v6-Visibility-Vertrag (freigegeben; Referenz: lokale Probe `probe2.html`, read-only — beide Zyklen nutzen denselben zentralen Zustandsvertrag, keine Sonderpatches pro Zeitstelle):**
- *Warnphase:* intakte Grundarena bleibt vollständig stabil; NUR die vorhandenen Risse kommen gestaffelt hinzu (gleiche `crackAt`-Ordnung wie der Abriss). Goldränder bleiben vollständig und durchgehend sichtbar (Gold weiterhin aus der Intact-Variante); Pfeiler, Wand, Gesimse, Sockel und Silhouette unverändert; keine Chips, weißen Bruchflächen, offenen Kanten oder Schlitze; keine vorzeitigen Visibility-, Material- oder Transform-Pops. Konkret je Segmentpaar: `cracked.visible = prog ≥ crackAt`, Chips (`M_Fracture_Cracked`-Teilmeshes) und Cracked-Gold unterdrückt, `intact` zeigt nur noch seine Gold-Teilmeshes, sobald das Cracked-Pendant sichtbar ist.
- *Collapse:* jedes Segment schaltet erst im ERSTEN sichtbar bewegten Frame (Tremor-Ende, `ts > COLV_TREMOR_S`) atomar auf seine vollständige Fallgeometrie — erst dann werden Chips, Bruchflächen und das segmentgebundene Gold mitbetroffen (Intact-Goldträger aus). Der Stage-2-Boden fällt **keilweise mit den Segmenten**: er liegt im Asset als `PlayFloor_Stage2_Wedge_01…06` vor, jeder Keil hängt in einer Pivot-Gruppe am Home-Punkt SEINES Segments und bekommt in jedem Zweig dieselbe Quaternion und dieselbe Position — dieselbe Verzögerung, dasselbe Taumeln, denselben Drift, denselben Fall. Kein zweiter Animationspfad, keine eigene Sichtbarkeitsregel, kein globales Ausblenden. Es bleibt zu keinem Zeitpunkt eine geschlossene Restfläche unter den fallenden Steinen stehen.
- *Geschlossene Rotationskörper:* `Tier2`/`GoldTier2` und `TempleTip`/`TipGold` kann kein Segment tragen. Sie dürfen deshalb nie sichtbar fallen und werden im ersten sichtbar bewegten Segment-Frame atomar ausgeblendet (`pedHide`), während die abkippenden Außensegmente den Wechsel verdecken. `Tier3`/`GoldTipBand` stehen ab Collapse 1 — also den GESAMTEN zweiten Zyklus — unbewegt an ihrer Endposition (`TIER3_LIFT`) luftspaltfrei unter dem Kern: kein Lift, kein Pop und kein Transform-Wechsel bei Collapse 2; der einmalige Umsprung liegt im Collapse-1-Frame, in dem Ring-1-Segmente und Sockelsturz die Sicht abdecken.

**Visuals-Produktumsetzung:** `assets/arena_platform_stage2.glb` ist die Produktarena. `PlayFloor` ist dort in `PlayFloor_Core` + `PlayFloor_Stage2_Wedge_01…06` geteilt, das restliche Knoteninventar ist identisch zu v1 (183 statt 178 Meshes, alle gemeinsamen Meshes unverändert). Ring 2 = zweite Instanz DESSELBEN freigegebenen Collapse-GLB mit einheitlicher Skalierung 0.82 — keine neue Ring-2-Geometrie. Collapse 2 betrifft die sechs Boden-Keile (fallen mit ihrem Segment), `Tier2` + `GoldTier2` und `TempleTip` + `TipGold` (atomar verdeckt); `Tier3` bleibt und trägt die finale Arena. Der Visual-State bleibt vollständig aus der Gameplay-Wahrheit abgeleitet; der instant/restore-Pfad stellt auch den direkten Sprung 0→Stage 2 dar (Rehydration/Late Join).

**Warum sechs Keile (Architekturentscheidung, 2026-08-03):** ein Rotationskörper kann nicht segmentweise zerfallen. Als monolithischer Ring blieb der Stage-2-Boden unter den fallenden Segmenten als geschlossene Fläche stehen und musste später global ausgeblendet werden — sichtbar als zweiter Pop. Kein Visibility-Timing kann das beheben, deshalb liegt die Lösung im Asset. `tools/split_arena_stage2.py` schneidet den Außenring in sechs echte Keile; die Fugen stammen aus dem freigegebenen Ring-1-Collapse-GLB selbst: die Segmentflanken verzahnen sich mit 1,6–2,1° Überlappung, die Fuge liegt jeweils in deren Mitte (keine hart kodierten Winkel). Benachbarte Schneidkörper teilen bitgleiche Randpunkte, der EXACT-Solver erzeugt daraus eine gemeinsame Ebene — punktweise deckungsgleich, Volumensumme 4,5e-10 relativ und Deckflächensumme 1,2e-07 relativ am Originalring. Topologie- und UV-Audit sind Teil des Builds und brechen den Export bei Verstoß ab.
...
**Umsetzungsstand (2026-08-03, lokal fertig, Collapse 2 visuell freigegeben):** vollständig implementiert in 9 lokalen Commits auf `feature/two-stage-collapse` (Vertrag → Stufen-Refactor → zweiter Zyklus lokal+online → Stage-2-Visuals inkl. `colvRing()`-Zentralvertrag → stufenbezogene Sound-Latches → Protocol v4 → Doku → Boden-Keile → Doku). Runner 17/17 grün (Ring-Collapse 359, Collapse-SFX 126, Reconnect-B2 103, Rules 195 mit v4-Fixtures). Architektur-Notizen: das v6-Inventar lässt Gesimse/Kapitelle/Pfeiler SICHTBAR stehen und mit dem Sockel stürzen (vorher ab Matchstart ausgeblendet), `WallRing` bleibt im Swap-Satz; die Keil-Segment-Kopplung ist im laufenden Match gemessen (2923 Frames, 17538 Paarvergleiche, null Abweichung über 1e-6), `attach()` erhält die Weltlage exakt. **Offen (nach Owner-Review):** Push, Live-Publish der v4-Rules auf `ringout-87fbb`, Production-Cutover (beide Clients müssen zeitgleich wechseln — v4 sperrt Altclients aus neuen Räumen aus).
