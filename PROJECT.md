...
- Server-seitige Sicherheit (implementiert, Publish/Verifikation ausstehend): `firebase.rules.json` (im Repo) erzwingt das Raum-Schema in der Realtime Database — **noch nicht in der Firebase Console publiziert**. Kernpunkte: Raumcode-Charset `[A-HJKMNP-Z2-9]{4}`; `v`/`config`/`created` nach Erstellung unveränderlich; sauberer Initial-Raum (gen 0, Host präsent, kein vorbefülltes `p/1` oder `g`); Züge write-once mit Wertegrenzen exakt wie `sanitizeMove` (idx 0–3, dx/dy ±195, sp ±1); Move-Writes nur für aktuelles `gen`; `gen` monoton (+1 oder idempotent gleich, 0–10 000); Presence mit 2-h-Join-Fenster; kein Root-Read → keine Raum-Enumeration. `created` nutzt `serverTimestamp()`, Rule `created === now` macht es server-autoritativ.
