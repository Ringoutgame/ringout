...
- **Kartenbilder.** `fbModusBild(bild)` erzeugt ein SVG (viewBox 160x100, `preserveAspectRatio
  slice`) aus den Spielkonstanten: Arenaform `poly` + Eckradius (runder Strich = Minkowski-Summe),
  Toroeffnungen und Torsockel in den Torfarben wie der Renderer, Startaufstellung aus denselben
  Spawn-Konstanten wie `placeBalls` (`fbBildAufstellung`: Tactical 1v1 `FOOTBALL_TACTICAL_1V1_SPAWN`,
  Team 2v2 `FOOTBALL_TACTICAL_SPAWN` mit `fbTeam2Side`, Tactical 4-Ball `FOOTBALL_TACTICAL4_SPAWN`,
  FFA `FOOTBALL_ARENA_ELIM5` (dirs x spawn), Classic `FOOTBALL_ARENA_CLASSIC.spawn`), Farben
  `FB_PCOLS`, Ball 25/32 im neutralen Slot, Zielhinweis je Zugmodell (`FB_BILDER`). Gesetzt einmal
  beim Boot (`fbKartenBilderSetzen`, nach `applyLang`, ausserhalb des Football-Blocks - die
  Pruefstaende schneiden den Block ohne DOM heraus). Die Suite prueft Bild == `placeBalls` je Modus.
...
Ringout/
  ...
  assets/hub/modes/  # Moduskarten-Bilder: 5 RingOut (genutzt), 5 Arena Football (seit 2026-09-22 unbenutzt - SVG aus Konstanten)
