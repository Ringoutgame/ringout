...
- **Kartenbilder.** Die Arena-Karten tragen die freigegebenen Renderer-Aufnahmen (700x438 WebP):
  ...
  `football_tactical4.webp` ist ein deterministisches Komposit aus `football_tactical.webp`
  (`artifacts/fb-menu-preview-01/tactical4_karte.js`: Farberkennung der fuenf Koerper, Homographie
  Spielflaeche -> Bild aus der Vier-Koerper-Aufstellung vom 9.9., Marmor-Retusche und Kugel-Sprites
  aus demselben Bild, Lagen aus `FOOTBALL_TACTICAL4_SPAWN`) - dieselbe Bildfamilie, kein Stilbruch.
  Ein am 22.9. versuchter SVG-Generator aus den Spielkonstanten wurde zurueckgenommen (wirkte
  schematisch); eine heutige Rohaufnahme des Renderers trifft die filmische Familie vom 9.9. nicht
  (`kartenbilder_tactical.js`, auch mit dem Himmel des 9.9.-Codes).
- **Raumvorschau.** Das Arena-Fenster des Onlinebildschirms (`.ohero`) ist die echte 3D-Szene.
  ...
  (`fbHeroRect`, `FB_VORSCHAU_ZOOM=0.92`); das Menue-Framing von Ring Out bleibt Zeichen fuer
